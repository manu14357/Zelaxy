...
export const requestTool: ToolConfig<RequestParams, RequestResponse> = {
  ...
  transformResponse: async (response: Response) => {
    ...
    if (isJson) {
      ...
      // Proxy responses wrap the real payload (status must be a number, not boolean)
      // This distinguishes Zelaxy proxy format {status: 200, data: ...} from external APIs returning {status: true, data: ...}
      if (
        jsonResponse &&
        jsonResponse.data !== undefined &&
        typeof jsonResponse.status === 'number'
      ) {
        return {
