...
import type {
  ...
  ParallelDeepResearchResponse,
} from '@/tools/parallel/types'
import type { ToolConfig } from '@/tools/types'

const logger = createLogger('ParallelDeepResearchTool')

export const deepResearchTool: ToolConfig<
  ParallelDeepResearchParams,
  ParallelDeepResearchResponse
> = {
  ...
  params: {
    ...
    processor: {
      ...
      description: 'Processing tier: lite, base, core, pro, ultra (default: pro)',
    },
    include_domains: {
      type: 'string',
      required: false,
  ...
  // The Task API returns a run_id immediately; poll its result endpoint once to return the completed
  // research (Parallel holds the connection until the run finishes).
  postProcess: async (result, params) => {
    ...
    try {
      ...
      logger.error('Error fetching research task result:', { message: errorMessage, runId })
  ...
  outputs: {
    status: { type: 'string', description: 'Task status (completed, failed, running)' },
    run_id: { type: 'string', description: 'Unique ID for this research task' },
    ...
    basis: {
      ...
      items: {
        ...
        properties: {
          ...
          citations: { type: 'array', description: 'Array of sources' },
