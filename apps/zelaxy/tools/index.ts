...
/**
 * Execute an MCP tool by routing to MCPService.
 * MCP tool IDs follow the format: mcp_<serverId>_<toolName>
 */
async function executeMCPTool(
  ...
  // Skip MCP execution on client-side
  if (typeof window !== 'undefined') {
    return {
      success: false,
      error: 'MCP tools can only be executed server-side',
      output: {},
      timing: {
  ...
  try {
    // Get MCPService from the registry (registered by API routes that can
    // safely import @/services/mcp without pulling child_process into client bundles)
    ...
    // Look up serverId and toolName from the registered mapping
    // (avoids fragile string parsing when serverIds contain underscores)
    const meta = getMCPToolMeta(toolId)
    if (!meta) {
      throw new Error(`Unknown MCP tool ID: ${toolId}. No mapping registered.`)
    }
    const { serverId, toolName } = meta

    logger.info(`[${requestId}] Executing MCP tool`, {
      serverId,
      toolName,
      hasParams: Object.keys(params).length > 0,
    })

    // Get user and workflow context
    const userId = params._context?.userId || (executionContext as any)?.userId || 'system'
    const workflowId = params._context?.workflowId || executionContext?.workflowId

    // Remove internal context params before passing to MCP
    const toolParams = { ...params }
    toolParams._context = undefined

    // For temporary servers (no DB record) use the connection directly;
    // for persistent servers use MCPService.executeTool (which also logs to DB).
    const connection = MCPService.getConnection(serverId)
    if (!connection) {
      throw new Error(`MCP server ${serverId} is not connected`)
    }

    const callResult = await connection.callTool(toolName, toolParams)

    const endTime = new Date()
    const duration = endTime.getTime() - startTime.getTime()

    logger.info(`[${requestId}] MCP tool execution successful`, {
      serverId,
      toolName,
      duration,
    })
    ...
    logger.error(`[${requestId}] MCP tool execution error`, {
      toolId,
      error: errorMessage,
      duration,
    })
...
export async function executeTool(
  ...
  try {
    // Handle MCP tool execution - route to MCPService
    if (toolId.startsWith(MCP_TOOL_PREFIX)) {
    ...
    // NOTE: Do NOT call processFileOutputs here.
...
function isErrorResponse(
  ...
  if (data && typeof data === 'object') {
    ...
    // If success is true, the error field is just informational data
    ...
    // If there's an error field but success is not false (could be true or undefined),
    // and there's no success field at all, then treat as error
...
async function handleInternalRequest(
  ...
  try {
    ...
    if (toolId === 'mssql_database') {
      console.log('[handleInternalRequest] MSSQL Response Check:', {
        toolId,
        isError,
        hasSuccess: 'success' in responseData,
        successValue: responseData.success,
        hasErrorField: 'error' in responseData,
        errorInfo,
      })
    }
