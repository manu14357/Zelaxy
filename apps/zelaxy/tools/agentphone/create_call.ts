...
export const agentphoneCreateCallTool: ToolConfig = {
  ...
  params: {
    ...
    fromNumberId: {
      ...
      required: false,
      visibility: 'user-or-llm',
      description: 'Phone number ID to use as caller ID',
    },
    initialGreeting: {
      type: 'string',
      required: false,
    ...
    voice: {
      ...
      required: false,
      visibility: 'user-or-llm',
      description: 'Voice ID override for this call',
    },
    systemPrompt: {
      type: 'string',
      required: false,
      visibility: 'user-or-llm',
      description: 'System prompt for built-in LLM conversation',
    },
  },
  ...
  transformResponse: async (response) => {
    ...
    if (!response.ok) {
      throw new Error(data?.detail?.[0]?.msg ?? data?.message ?? 'Failed to create call')
    }
    return {
      success: true,
      output: {
  ...
  outputs: {
    ...
    phoneNumberId: { type: 'string', description: 'ID of the phone number used', optional: true },
