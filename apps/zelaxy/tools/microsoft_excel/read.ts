...
export const readTool: ToolConfig<MicrosoftExcelToolParams, MicrosoftExcelReadResponse> = {
  ...
  request: {
    url: (params) => {
      ...
      let baseUrl
      if (!params.range) {
        baseUrl = `https://graph.microsoft.com/v1.0/me/drive/items/${spreadsheetId}/workbook/worksheets('Sheet1')/range(address='A1:Z1000')`
      } else {
        ...
        baseUrl = `https://graph.microsoft.com/v1.0/me/drive/items/${spreadsheetId}/workbook/worksheets('${sheetName}')/range(address='${address}')`
      }

      // Add query parameters to get both values and formatted text
      return `${baseUrl}?$select=address,values,text,numberFormat`
    },
    method: 'GET',
    headers: (params) => {
  ...
  transformResponse: async (response: Response) => {
    ...
    // Use formatted text values if available, otherwise fall back to raw values
    const formattedValues = data.text || data.values || []

    const result: MicrosoftExcelReadResponse = {
      success: true,
      output: {
        data: {
          range: data.address || data.range || '',
