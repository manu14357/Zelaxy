...
export const amplitudeSendEventTool: ToolConfig<
  AmplitudeSendEventParams,
  AmplitudeSendEventResponse
> = {
  ...
  params: {
    ...
    userProperties: {
      ...
      required: false,
      visibility: 'user-or-llm',
      description: 'JSON object of user properties to set',
    },
    time: {
      type: 'string',
      required: false,
  ...
  request: {
    ...
    body: (params) => {
      ...
      return { api_key: params.apiKey, events: [event] }
    },
  },
  ...
  outputs: {
    code: { type: 'number', description: 'Response code (200 for success)' },
    eventsIngested: { type: 'number', description: 'Number of events ingested' },
    payloadSizeBytes: { type: 'number', description: 'Size of the payload in bytes' },
    serverUploadTime: { type: 'number', description: 'Server upload timestamp' },
  },
}
