...
export interface AsanaGetTaskResponse extends ToolResponse {
  output: {
    ...
    created_by?: { gid: string; resource_type: string; name: string }
    due_on?: string
    created_at?: string
    modified_at?: string
    tasks?: Array<{
    ...
    next_page?: { offset: string; path: string; uri: string }
  }
}
...
export interface AsanaGetProjectsResponse extends ToolResponse {
  output: {
    ts: string
    projects: Array<{ gid: string; name: string; resource_type: string }>
  }
}
