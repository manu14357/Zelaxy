...
export const agentmailReplyMessageTool: ToolConfig = {
  ...
  request: {
    ...
    body: (params) => {
      ...
      if (!params.replyAll) {
        if (params.to) body.to = params.to.split(',').map((e: string) => e.trim())
  ...
  transformResponse: async (response) => {
    ...
    if (!response.ok) {
      throw new Error(data.message ?? 'Failed to reply to message')
    }
    return {
      success: true,
      output: {
