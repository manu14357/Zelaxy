...
export const ahrefsBacklinksTool: ToolConfig<AhrefsBacklinksParams, AhrefsBacklinksResponse> = {
  id: 'ahrefs_backlinks',
  ...
  transformResponse: async (response: Response) => {
    ...
    const backlinks = (data.backlinks || []).map((link: Record<string, unknown>) => ({
      urlFrom: (link.url_from as string) || '',
      urlTo: (link.url_to as string) || '',
      anchor: (link.anchor as string) || '',
      domainRatingSource:
        (link.domain_rating_source as number) ?? (link.domain_rating as number) ?? 0,
      isDofollow: (link.is_dofollow as boolean) ?? (link.dofollow as boolean) ?? false,
      firstSeen: (link.first_seen as string) || '',
      lastVisited: (link.last_visited as string) || '',
    }))
    return { success: true, output: { backlinks } }
  },
