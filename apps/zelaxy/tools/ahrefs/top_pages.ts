...
export const ahrefsTopPagesTool: ToolConfig<AhrefsTopPagesParams, AhrefsTopPagesResponse> = {
  id: 'ahrefs_top_pages',
  ...
  transformResponse: async (response: Response) => {
    ...
    const pages = (data.pages || data.top_pages || []).map((page: Record<string, unknown>) => ({
      url: (page.url as string) || '',
      traffic: (page.traffic as number) ?? 0,
      keywords: (page.keywords as number) ?? (page.keyword_count as number) ?? 0,
      topKeyword: (page.top_keyword as string) || '',
      value: (page.value as number) ?? (page.traffic_value as number) ?? 0,
    }))
    return { success: true, output: { pages } }
  },
