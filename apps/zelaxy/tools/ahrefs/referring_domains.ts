...
export const ahrefsReferringDomainsTool: ToolConfig<
  AhrefsReferringDomainsParams,
  AhrefsReferringDomainsResponse
> = {
  ...
  transformResponse: async (response: Response) => {
    ...
    const referringDomains = (data.refdomains || data.referring_domains || []).map(
      (domain: Record<string, unknown>) => ({
        domain: (domain.domain as string) || (domain.refdomain as string) || '',
        domainRating: (domain.domain_rating as number) ?? 0,
        backlinks: (domain.backlinks as number) ?? 0,
        dofollowBacklinks:
          (domain.dofollow_backlinks as number) ?? (domain.dofollow as number) ?? 0,
        firstSeen: (domain.first_seen as string) || '',
        lastVisited: (domain.last_visited as string) || '',
      })
    )
    return { success: true, output: { referringDomains } }
  },
