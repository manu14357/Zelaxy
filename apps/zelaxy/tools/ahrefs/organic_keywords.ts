...
export const ahrefsOrganicKeywordsTool: ToolConfig<
  AhrefsOrganicKeywordsParams,
  AhrefsOrganicKeywordsResponse
> = {
  ...
  transformResponse: async (response: Response) => {
    ...
    const keywords = (data.keywords || data.organic_keywords || []).map(
      (kw: Record<string, unknown>) => ({
        ...
        position: (kw.position as number) ?? 0,
        url: (kw.url as string) || '',
        traffic: (kw.traffic as number) ?? 0,
        keywordDifficulty: (kw.keyword_difficulty as number) ?? (kw.difficulty as number) ?? 0,
      })
    )
    return { success: true, output: { keywords } }
  },
    ...
    keywords: {
      ...
      items: {
        ...
        properties: {
          ...
          keywordDifficulty: { type: 'number', description: 'Keyword difficulty score (0-100)' },
        },
      },
    },
  },
