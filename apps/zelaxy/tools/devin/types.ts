...
export interface DevinAddSecretParams {
  apiKey: string
  secretName: string
  secretValue: string
}

export interface DevinDeleteSecretParams {
  apiKey: string
  secretName: string
}

export interface DevinGetSnapshotParams {
  apiKey: string
  sessionId: string
}
...
export const DEVIN_SESSION_LIST_ITEM_PROPERTIES = {
  ...
  url: { type: 'string', description: 'URL to view the session' },
  status: { type: 'string', description: 'Session status' },
  statusDetail: { type: 'string', description: 'Detailed status', optional: true },
  title: { type: 'string', description: 'Session title', optional: true },
  createdAt: { type: 'number', description: 'Creation timestamp (Unix)', optional: true },
  updatedAt: { type: 'number', description: 'Last updated timestamp (Unix)', optional: true },
...
export interface DevinAddSecretResponse extends ToolResponse {
  output: {
    success: boolean
...
export interface DevinDeleteSecretResponse extends ToolResponse {
  output: {
    success: boolean
    secretName: string | null
  }
...
export interface DevinGetSnapshotResponse extends ToolResponse {
  output: {
    sessionId: string | null
