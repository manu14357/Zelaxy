import type {
  AlgoliaUpdateDocumentParams,
  AlgoliaUpdateDocumentResponse,
} from '@/tools/algolia/types'
import type { ToolConfig } from '@/tools/types'

export const algoliaUpdateDocumentTool: ToolConfig<
  AlgoliaUpdateDocumentParams,
  AlgoliaUpdateDocumentResponse
> = {
  id: 'algolia_update_document',
  name: 'Algolia Update Record',
  ...
  params: {
    ...
    attributes: {
      ...
      required: true,
      visibility: 'user-or-llm',
      description: 'JSON object with attributes to update',
    },
    createIfNotExists: {
      type: 'boolean',
      required: false,
  ...
  outputs: {
    taskID: { type: 'number', description: 'Algolia task ID for tracking the update operation' },
    objectID: { type: 'string', description: 'The objectID of the updated record' },
    updatedAt: { type: 'string', description: 'Timestamp when the record was updated' },
  },
}
