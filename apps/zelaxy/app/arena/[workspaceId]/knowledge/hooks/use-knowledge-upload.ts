        ...
        try {
          ...
          if (presignedData.directUploadSupported) {
            ...
            try {
              ...
              if (fetchError instanceof DirectUploadError) {
                throw fetchError
              }

              // Handle network errors (like CORS failures)
              throw new DirectUploadError(
                `Network error uploading ${file.name}: ${fetchError instanceof Error ? fetchError.message : 'Unknown error'}`,
                {
                  originalError:
