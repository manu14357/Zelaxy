...
import {
  ...
  DocumentLoading,
  EditChunkModal,
} from '@/app/arena/[workspaceId]/knowledge/[id]/[documentId]/components'
import { ActionBar } from '@/app/arena/[workspaceId]/knowledge/[id]/components'
import { KnowledgeHeader, SearchInput } from '@/app/arena/[workspaceId]/knowledge/components'
...
export function Document({
            ...
            <div className='flex items-center gap-2'>
              ...
              <span className='text-muted-foreground text-sm italic'>
                {documentData?.processingStatus === 'pending' &&
                  'Document processing is pending...'}
                {documentData?.processingStatus === 'processing' &&
                  'Document is being processed...'}
                {documentData?.processingStatus === 'failed' && 'Document processing has failed'}
                {!documentData?.processingStatus && 'Document is not ready'}
              </span>
            </div>
          </td>
          <td className='px-4 py-3'>
  ...
  const breadcrumbs = [
    { label: 'Knowledge Hub', href: `/arena/${workspaceId}/hub?tab=knowledge` },
    {
      ...
      <div className='flex h-screen flex-col bg-gradient-to-br from-background to-muted/20'>
        ...
        <div className='flex flex-1 items-center justify-center'>
          <div className='text-center'>
            <div className='rounded-lg border border-red-200 bg-gradient-to-r from-red-50 to-red-50/80 p-6 shadow-sm'>
              <p className='mb-3 font-medium text-red-800 text-sm'>
                Failed to load document: {combinedError}
              </p>
              <button
                onClick={() => window.location.reload()}
                className='font-medium text-red-600 text-sm underline decoration-2 underline-offset-2 transition-colors hover:text-red-700'
              >
                Retry Loading
              </button>
            </div>
            ...
            <div className='px-6 pb-8'>
              ...
              <div className='mb-6 flex items-center justify-between pt-2'>
                ...
                <Button
                  ...
                  size='sm'
                  className='flex items-center gap-1 bg-primary font-[480] text-primary-foreground shadow-md transition-shadow duration-200 hover:bg-primary/90 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50'
                >
              ...
              {combinedError && (
                <div className='mb-6 rounded-lg border border-red-200 bg-gradient-to-r from-red-50 to-red-50/80 p-4 shadow-sm'>
                  <p className='font-medium text-red-800 text-sm'>
                    Failed to load chunks: {combinedError}
                  </p>
                  <button
                    onClick={() => window.location.reload()}
                    className='mt-2 font-medium text-red-600 text-sm underline decoration-2 underline-offset-2 transition-colors hover:text-red-700'
                  >
                    Retry Loading
                  </button>
                </div>
...

              {
                ...
                <div className='flex flex-1 flex-col overflow-hidden'>
                  ...
                  {documentData?.processingStatus === 'completed' && totalPages > 1 && (
                    <div className='mt-6 flex items-center justify-center rounded-lg border bg-card px-6 py-4 shadow-sm'>
                      <div className='flex items-center gap-1'>
                        <Button
                          variant='ghost'
                          size='sm'
