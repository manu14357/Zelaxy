...
import { MiniMap } from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/minimap/minimap'
import { Panel } from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/panel/panel'
...
import { WorkflowTextEditorModal } from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/workflow-text-editor/workflow-text-editor-modal'
...
// Warning notification component for displaying workflow warnings (e.g., nested subflows)
const WorkflowWarningNotification = React.memo(() => {
  const [warning, setWarning] = useState<{ type: string; message: string } | null>(null)

  useEffect(() => {
    const handleWarning = (event: CustomEvent<{ type: string; message: string }>) => {
      setWarning(event.detail)
      // Auto-dismiss after 5 seconds
      setTimeout(() => setWarning(null), 5000)
    }

    window.addEventListener('workflow-warning', handleWarning as EventListener)
    return () => window.removeEventListener('workflow-warning', handleWarning as EventListener)
  }, [])

  if (!warning) return null

  return (
    <div className='-translate-x-1/2 fade-in slide-in-from-top-2 fixed top-4 left-1/2 z-[100] animate-in duration-300'>
      <div className='flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 shadow-lg dark:border-amber-800 dark:bg-amber-950'>
        <AlertTriangle className='h-5 w-5 flex-shrink-0 text-amber-600 dark:text-amber-400' />
        <div className='flex-1'>
          <p className='font-medium text-amber-800 text-sm dark:text-amber-200'>
            Action Not Allowed
          </p>
          <p className='text-amber-700 text-xs dark:text-amber-300'>{warning.message}</p>
        </div>
        <button
          onClick={() => setWarning(null)}
          className='rounded p-1 text-amber-600 transition-colors hover:bg-amber-100 dark:text-amber-400 dark:hover:bg-amber-900'
...
WorkflowWarningNotification.displayName = 'WorkflowWarningNotification'
...
const WorkflowContent = React.memo(() => {
  ...
  // Resolve theme for ReactFlow colorMode (watches document.documentElement class)
    ...
    (event: React.DragEvent) => {
      ...
      try {
        ...
        if (data.type === 'loop' || data.type === 'parallel') {
          // BLOCK nested subflows - don't allow loop/parallel inside another container
          if (containerInfo) {
            logger.warn(
              'Nested subflows not allowed - blocking drop of',
              data.type,
              'inside container',
              containerInfo.loopId
            )
            // Dispatch custom event to show warning notification
            window.dispatchEvent(
              new CustomEvent('workflow-warning', {
                detail: {
                  type: 'nested-subflow',
                  message:
                    'Nested subflows are not supported. Loop and Parallel blocks cannot be placed inside other Loop or Parallel blocks.',
                },
              })
            )
            return
          ...
          // Container nodes can only be added at canvas level (not nested)
          {
  ...
  const onDragOver = useCallback(
    ...
    [screenToFlowPosition, isPointInLoopNodeWrapper, getNodes]
  )
  ...
  useEffect(() => {
    const validateAndNavigate = async () => {
      ...
      if (workflowIds.length === 0 && !isLoading) {
        ...
        router.replace(`/arena/${workspaceId}/zelaxy`)
        return
      }
      ...
      if (currentWorkflow && currentWorkflow.workspaceId !== workspaceId) {
        ...
        router.replace(`/arena/${currentWorkflow.workspaceId}/zelaxy/${currentId}`)
        return
      }
  ...
  const nodes = useMemo(() => {
    ...
    // Create a set of valid block IDs to validate parent references
    // React Flow requires parent nodes to exist when parentId is set
    const validBlockIds = new Set(Object.keys(blocks))

    // Helper to get validated parentId - returns undefined if parent doesn't exist
    const getValidatedParentId = (blockId: string, parentId: string | undefined) => {
      if (!parentId) return undefined
      if (validBlockIds.has(parentId)) return parentId
      // Parent doesn't exist - log warning and skip the parentId to prevent React Flow error
      logger.warn('Skipping invalid parent reference during node creation', {
        nodeId: blockId,
    ...
    Object.entries(blocks).forEach(([blockId, block]) => {
      ...
      const validatedParentId = getValidatedParentId(blockId, block.data?.parentId)
      // Only set extent if we have a valid parent
      const validatedExtent = validatedParentId ? block.data?.extent || undefined : undefined
  ...
  const onConnect = useCallback(
    (connection: any) => {
      if (connection.source && connection.target) {
        ...
        if ((targetNode.data?.config as any)?.category === 'triggers') {
          return
        }
  ...
  return (
    <div className='flex h-screen w-full flex-col overflow-hidden'>
      {/* Global warning notification for workflow errors */}
      <WorkflowWarningNotification />
      ...
      <div className='relative h-full w-full flex-1 transition-all duration-200'>
        ...
        <ReactFlow
          ...
          connectionLineStyle={{
            ...
            strokeLinecap: 'round',
            strokeLinejoin: 'round',
            filter: 'drop-shadow(0 1px 3px rgba(249,115,22,0.2))',
          }}
        ...
        >
          {/* Clean dot grid — Figma/Notion-inspired minimal spacing */}
          <Background
            id='dots-primary'
            variant={BackgroundVariant.Dots}
            color='hsl(var(--workflow-dots))'
        ...
        {/* Workflow Text Editor (YAML/JSON) */}
