...
interface ExecutorOptions {
  ...
  contextExtensions?: {
    ...
    executionId?: string
    workspaceId?: string
    onBlockComplete?: (blockLog: any) => void | Promise<void>
    onExecutionStart?: (workflowId?: string, executionId?: string) => void | Promise<void>
    onExecutionComplete?: (result: any) => void | Promise<void>
  }
}
...
export function useWorkflowExecution() {
                ...
                for (const fileData of workflowInput.files) {
                  ...
                  if (response.ok) {
                    ...
                    const processUploadResult = (result: any) => ({
                      ...
                      ...(result.parsedContent && { parsedContent: result.parsedContent }),
                    })
  ...
  const executeWorkflow = async (
    ...
    // Regenerate loops and parallels from the merged states so that
    // subblock values (e.g. iterations count) are up-to-date.
    // The store's workflowLoops may still have stale default values
    // because subblock changes are only synced to block.data during merge.
    const freshLoops = generateLoopBlocks(filteredStates)
    const freshParallels = generateParallelBlocks(filteredStates)
