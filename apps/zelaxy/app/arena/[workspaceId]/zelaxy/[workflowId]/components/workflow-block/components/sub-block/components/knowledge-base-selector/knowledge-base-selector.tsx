      ...
      {isMultiSelect && selectedKnowledgeBases.length > 0 && (
        <div className='mb-2 flex flex-wrap gap-1'>
          {selectedKnowledgeBases.map((kb) => (
            <div
              key={kb.id}
              className='inline-flex items-center rounded-md border border-primary/20 bg-primary/10 px-2 py-1 text-xs'
            >
              <PackageSearchIcon className='mr-1 h-3 w-3 text-primary' />
              <span className='font-medium text-primary'>{formatKnowledgeBaseName(kb)}</span>
              {!disabled && !isPreview && (
                <button
