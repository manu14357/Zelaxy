...
export function ShortInput({
  ...
  const handleDrop = (e: React.DragEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const dropPosition = inputRef.current?.selectionStart ?? value?.toString().length ?? 0 // Insert '{{' at drop position to trigger the dropdown
      ...
      Promise.resolve().then(() => {
        ...
        setTimeout(() => {
          if (inputRef.current) {
            inputRef.current.selectionStart = dropPosition + 2
            inputRef.current.selectionEnd = dropPosition + 2
          }
        }, 0)
      })
    } catch (error) {
