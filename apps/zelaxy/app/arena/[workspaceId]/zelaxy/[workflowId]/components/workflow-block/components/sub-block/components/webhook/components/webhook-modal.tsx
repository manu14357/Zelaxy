...
import {
  type ProviderConfig,
  WEBHOOK_PROVIDERS,
} from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/workflow-block/components/sub-block/components/webhook/webhook'
...
export function WebhookModal({
  ...
  // Construct the full webhook URL — use empty fallback during SSR, populate on client
  const [baseUrl, setBaseUrl] = useState('')
  useEffect(() => {
    setBaseUrl(`${window.location.protocol}//${window.location.host}`)
  }, [])

  const webhookUrl = baseUrl ? `${baseUrl}/api/webhooks/trigger/${formattedPath}` : ''
