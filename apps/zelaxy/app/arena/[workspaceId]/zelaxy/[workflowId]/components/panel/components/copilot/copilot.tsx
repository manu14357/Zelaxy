...
import {
  ...
  CopilotWelcome,
  UserInput,
} from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/panel/components/copilot/components'
import type {
...
export const Copilot = forwardRef<CopilotRef, CopilotProps>(({ panelWidth }, ref) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('copilot-new-chat', handleNewChatEvent)
    return () => window.removeEventListener('copilot-new-chat', handleNewChatEvent)
  }, [handleStartNewChat])
            ...
            {showCheckpoints ? (
              ...
              <div className='relative flex-1 overflow-hidden'>
                <ScrollArea ref={scrollAreaRef} className='h-full' hideScrollbar={true}>
                  <div className='w-full max-w-full space-y-2 overflow-hidden p-2'>
                    {messages.length === 0 ? (
                ...
                {showScrollButton && (
                  <div className='-translate-x-1/2 absolute bottom-3 left-1/2 z-10'>
                    <Button
                      ...
                      size='sm'
                      variant='outline'
                      className='flex items-center gap-1 rounded-full border border-gray-200 bg-white px-2 py-1 shadow-lg transition-all hover:bg-gray-50'
                    >
