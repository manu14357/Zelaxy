...
import {
  ...
  Table,
  TimeInput,
} from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/workflow-block/components/sub-block/components'
import {
  type CustomTool,
  CustomToolModal,
} from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/workflow-block/components/sub-block/components/tool-input/components/custom-tool-modal/custom-tool-modal'
import { ToolCommand } from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/workflow-block/components/sub-block/components/tool-input/components/tool-command/tool-command'
import { ToolCredentialSelector } from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/workflow-block/components/sub-block/components/tool-input/components/tool-credential-selector'
...
function GenericSyncWrapper<T = unknown>({
  ...
  const [storeValue, setStoreValue] = useSubBlockValue(blockId, paramId)

  // Sync FROM store → parent (when user edits via the component)
  useEffect(() => {
    if (storeValue) {
  ...
  // Sync FROM parent → store (restore saved values on mount/reload)
  // This ensures the component's internal store is initialized from
  // the persisted tool params value when the page reloads.
  useEffect(() => {
    if (value && !storeValue) {
      setStoreValue(value)
    }
  }, [value, storeValue, setStoreValue])
...
export function ToolInput({
  ...
  const handleSelectTool = (toolBlock: (typeof toolBlocks)[0]) => {
    ...
    toolParams.userInputParameters.forEach((param) => {
      if (param.uiComponent?.value !== undefined && !initialParams[param.id]) {
        const defaultValue =
          typeof param.uiComponent.value === 'function'
            ? param.uiComponent.value()
            : param.uiComponent.value
        // Always set defaults, even falsy ones (e.g., false for switches)
        if (defaultValue !== undefined && defaultValue !== null) {
          initialParams[param.id] = String(defaultValue)
        }
      }
      // Also set defaults from tool param definition
      if (!initialParams[param.id] && param.default !== undefined) {
  ...
  const handleOperationChange = (toolIndex: number, operation: string) => {
    ...
    // Apply default values from UI component configurations
    toolParams.userInputParameters.forEach((param) => {
      if (param.uiComponent?.value !== undefined && !initialParams[param.id]) {
        const defaultValue =
          typeof param.uiComponent.value === 'function'
  ...
  const evaluateParameterCondition = (param: any, tool: StoredTool): boolean => {
    ...
    // Helper: normalize a field value for condition comparison.
    // Switches in tool-input store 'true'/'false' strings, but conditions
    // use boolean true/false. Coerce string booleans and null/undefined.
    const normalizeValue = (raw: any, conditionValue: any): any => {
      if (raw === undefined || raw === null) {
        return typeof conditionValue === 'boolean' ? false : raw
      }
    ...
    const fieldValue = normalizeValue(currentValues[condition.field], condition.value)
    let result = false
    ...
    if (condition.and) {
      const andFieldValue = normalizeValue(currentValues[condition.and.field], condition.and.value)
      let andResult = false
  ...
  const renderParameterInput = (
    ...
    switch (uiComponent.type) {
      case 'dropdown': {
        // Resolve options (may be a function or static array)
        let options: { id: string; label: string }[] = []
        if (typeof uiComponent.options === 'function') {
          try {
            options =
              (uiComponent.options as unknown as () => { id: string; label: string }[])() || []
          } catch {
            options = []
          }
        } else if (Array.isArray(uiComponent.options)) {
          options = uiComponent.options as unknown as { id: string; label: string }[]
        }

        // If no options available (dynamic dropdowns like MCP server/tool selection),
        // fall back to a text input so the user can type a value
        if (options.length === 0) {
          return (
            <ShortInput
              blockId={uniqueBlockId}
              subBlockId={`${subBlockId}-param`}
              placeholder={uiComponent.placeholder || param.description || `Enter ${param.id}`}
              isConnecting={false}
              config={{
                id: `${subBlockId}-param`,
                type: 'short-input',
                      ...
                      <div
                        ...
                        style={{
                          backgroundColor: isCustomTool
                            ? 'hsl(var(--primary))' // primary for custom tools
                            : toolBlock?.bgColor,
                        }}
                      >
                        {isCustomTool ? (
