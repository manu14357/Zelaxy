...
import type { AuthType } from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/control-bar/components/deploy-modal/components/chat-deploy/hooks/use-chat-form'
          ...
          <CardContent className='p-4'>
            ...
            {isExistingChat && !password && (
              <div className='mb-2 flex items-center text-muted-foreground text-xs'>
                <div className='mr-2 bg-primary/10 px-2 py-0.5 font-medium text-primary'>
                  Password set
                </div>
      ...
      {authType === 'email' && (
        <Card className='shadow-none'>
          <CardContent className='p-4'>
            ...
            {emails.length > 0 && (
              <div className='mt-3 max-h-[150px] overflow-y-auto border bg-background px-2 py-0 shadow-none'>
                <ul className='divide-y divide-border'>
                  {emails.map((email) => (
                    <li key={email} className='relative'>
                      <div className='group my-1 flex items-center justify-between px-2 py-2 text-sm'>
