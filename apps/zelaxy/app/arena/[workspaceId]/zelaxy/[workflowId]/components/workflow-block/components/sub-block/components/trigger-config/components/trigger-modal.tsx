...
export function TriggerModal({
  ...
  // For triggers with no config fields (e.g., Generic Webhook), always allow saving
  // when there's no existing trigger yet (no triggerId)
  const hasConfigChanged = useMemo(() => {
    const configChanged = JSON.stringify(config) !== JSON.stringify(initialConfig)
    const hasNoConfigFields = Object.keys(triggerDef.configFields).length === 0
    const isNewTrigger = !triggerId
    return configChanged || (hasNoConfigFields && isNewTrigger)
  }, [config, initialConfig, triggerDef.configFields, triggerId])
  ...
  useEffect(() => {
    ...
    // If no path exists, generate one automatically — but only once
    // (avoid regenerating on every re-render which causes URL mismatch)
    if (!finalPath) {
      if (generatedPath) {
