...
import {
  CodeDisplay,
  JSONView,
} from '@/app/arena/[workspaceId]/zelaxy/[workflowId]/components/panel/components/console/components'
  ...
  return (
    <div className='space-y-3'>
      ...
      <div className='flex flex-wrap items-center gap-1.5'>
        <div
          className={`flex h-5 items-center rounded-md px-2 ${
            entry.error ? 'bg-destructive/10 dark:bg-destructive/15' : 'bg-muted dark:bg-muted'
          }`}
        >
          {entry.error ? (
            <div className='flex items-center gap-1'>
              <AlertCircle className='h-3 w-3 text-destructive dark:text-destructive' />
              <span className='font-medium text-destructive text-xs leading-normal dark:text-destructive'>
                Error
              </span>
            </div>
          ) : (
            <span className='font-medium text-muted-foreground text-xs leading-normal'>
              {entry.durationMs ?? 0}ms
            </span>
          )}
        </div>
        <div className='flex h-5 items-center rounded-md bg-muted px-2 dark:bg-muted'>
          <span className='font-medium text-muted-foreground text-xs leading-normal'>
        ...
        {hasInputData && (
          <>
            <button
              ...
              className={`flex h-5 items-center rounded-md px-2 transition-colors ${
                !showInput
                  ? 'bg-primary/10 text-primary dark:bg-primary/15 dark:text-primary'
                  : 'bg-muted text-muted-foreground hover:bg-accent hover:text-foreground dark:bg-muted'
              }`}
            >
              <span className='font-medium text-xs leading-normal'>Output</span>
            </button>
            <button
            ...
            >
              <span className='font-medium text-xs leading-normal'>Input</span>
            </button>
      ...
      <div className='space-y-2 pb-2'>
        ...
        {entry.error && !showInput && (
          <div className='bg-destructive/20 p-3 dark:bg-destructive/20'>
            <div className='overflow-hidden whitespace-pre-wrap break-all font-normal text-destructive text-sm leading-normal dark:text-destructive'>
              {entry.error}
            </div>
          </div>
        )}
        ...
        {entry.warning && !showInput && (
          <div className='border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800/50'>
        ...
        {(showInput ? hasInputData : entry.output != null && !entry.error) && (
          <div
            className={`rounded-lg border border-border/40 bg-muted/50 dark:bg-muted/30 ${shouldShowCodeDisplay ? 'p-0' : 'p-3'}`}
          >
            {shouldShowCodeDisplay ? (
              ...
              <div className='relative'>
                ...
                {/* Content */}
                {isExpanded ? (
                  <div className='max-w-full overflow-x-auto break-words font-mono font-normal text-foreground/80 text-sm leading-relaxed'>
                    <JSONView data={displayData} />
                  </div>
                ) : (
                  <div
                    className='max-w-full cursor-pointer overflow-hidden break-words font-mono font-normal text-foreground/60 text-sm leading-relaxed'
        ...
        {!showInput && entry.output == null && !entry.error && (
          <div className='rounded-lg border border-border/40 bg-muted/50 p-3 dark:bg-muted/30'>
