...
export function SubBlock({
  ...
  // Only enable MCP hooks for MCP-specific sub-blocks to avoid unnecessary API calls
  const isMCPServerField = config.id === 'existingServerId'
  const isMCPToolField = config.id === 'selectedTool'

  // Load MCP servers for dropdown population (only when rendering the server dropdown)
  const { serverOptions, loading: mcpLoading } = useMCPServers(isMCPServerField)

  // Get the selected server ID from subblock values for tool loading
  const selectedServerId = subBlockValues?.existingServerId as string | undefined

  // Load MCP tools for the selected server (only when rendering the tool dropdown)
  const { toolOptions, loading: toolsLoading } = useMCPTools(selectedServerId, isMCPToolField)

  // Utility function to safely resolve options
  const resolveOptions = (options: any): { label: string; id: string }[] => {
    try {
      ...
      console.warn('Failed to resolve options:', error)
      return []
    }
  }
  ...
  const renderInput = () => {
    ...
    switch (config.type) {
      ...
      case 'dropdown': {
        // Special handling for MCP server selection and tool selection
        let dropdownOptions: { id: string; label: string }[] = []
        let dropdownDisabled = isDisabled

        if (config.id === 'existingServerId') {
          // MCP Server dropdown
          dropdownOptions = mcpLoading
            ? [{ id: 'loading', label: 'Loading servers...' }]
            : serverOptions
          dropdownDisabled = isDisabled || mcpLoading
        } else if (config.id === 'selectedTool') {
          // MCP Tool dropdown - only show if a server is selected
          if (!selectedServerId) {
            dropdownOptions = [{ id: '', label: 'Select a server first...' }]
            dropdownDisabled = true
          } else if (toolsLoading) {
            dropdownOptions = [{ id: 'loading', label: 'Loading tools...' }]
            dropdownDisabled = true
          } else if (toolOptions.length === 0) {
            dropdownOptions = [{ id: '', label: 'No tools available' }]
            dropdownDisabled = true
          } else {
            dropdownOptions = toolOptions
            dropdownDisabled = isDisabled
          }
        } else {
          // Regular dropdown
          dropdownOptions = resolveOptions(config.options)
        }
