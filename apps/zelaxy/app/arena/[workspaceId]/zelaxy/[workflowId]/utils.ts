...
const getBlockDimensions = (
  ...
  const block = blocks[blockId]
  if (!block) return { width: 180, height: 60 }
  ...
  return {
    width: block.isWide ? 240 : block.data?.width || block.width || 180,
    height: Math.max(block.height || block.data?.height || 60, 60),
  }
}
 ...
 * at apps/core/lib/autolayout/ - all auto layout now goes through the API routes
