...
} from '@/app/arena/[workspaceId]/providers/workspace-permissions-provider'
...
export function InviteModal({ open, onOpenChange, workspaceName }: InviteModalProps) {
  ...
  const fetchPendingInvitations = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/arenas/invitations')
      if (response.ok) {
  ...
  const handleRemoveMemberConfirm = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/arenas/members/${memberToRemove.userId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleRemoveInvitationConfirm = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/arenas/invitations/${invitationToRemove.invitationId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <AlertDialogFooter className='flex justify-between'>
            ...
            <Button
              ...
              className={cn(
                ...
                'bg-primary text-white shadow-[0_0_0_0_hsl(var(--primary))] hover:bg-primary/90 hover:shadow-[0_0_0_4px_hsl(var(--primary)/0.15)] disabled:opacity-50 disabled:hover:bg-primary disabled:hover:shadow-none'
              )}
            >
