...
import {
  ...
  TeamMembersList,
  TeamSeatsOverview,
} from '@/app/arena/[workspaceId]/zelaxy/components/sidebar/components/settings-modal/components/team-management/components'
...
export function TeamManagement() {
  ...
  // Set default organization name for team/enterprise users (or when billing is disabled)
  useEffect(() => {
  ...
  const handleCreateOrganization = useCallback(async () => {
    ...
    // Link the current workspace to the newly created organization
    const newOrgId = useOrganizationStore.getState().activeOrganization?.id
    if (newOrgId && params.workspaceId) {
      try {
        await fetch(`/api/arenas/${params.workspaceId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ organizationId: newOrgId }),
        })
        logger.info('Linked workspace to organization', {
          workspaceId: params.workspaceId,
          organizationId: newOrgId,
        })
      } catch (error) {
        logger.error('Failed to link workspace to organization', { error })
      }
    }
    ...
    setOrgName('')
    setOrgSlug('')
  }, [session?.user?.id, orgName, orgSlug, params.workspaceId])
    ...
    <div className='space-y-5 px-3 py-4 sm:px-5 sm:py-6'>
        ...
        action={
          organizations.length > 1 ? (
            <select
              ...
              className='h-8 rounded-lg border border-border/60 bg-background px-3 text-[13px]'
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className='h-9 rounded-lg bg-muted/60 p-0.5'>
          <TabsTrigger value='members' className='rounded-md text-[13px]'>
            Members
          </TabsTrigger>
          <TabsTrigger value='usage' className='rounded-md text-[13px]'>
            Usage
          </TabsTrigger>
          <TabsTrigger value='settings' className='rounded-md text-[13px]'>
            Settings
          </TabsTrigger>
        </TabsList>
