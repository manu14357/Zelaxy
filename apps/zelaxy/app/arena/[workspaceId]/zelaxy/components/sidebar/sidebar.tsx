...
import {
  ...
  WorkspaceHeader,
  WorkspaceSelector,
} from '@/app/arena/[workspaceId]/zelaxy/components/sidebar/components'
import { InviteModal } from '@/app/arena/[workspaceId]/zelaxy/components/sidebar/components/workspace-selector/components/invite-modal/invite-modal'
import {
...
const SIDEBAR_HEIGHTS = {
  ...
  NAVIGATION: 44, // h-9 buttons + p-1 container border
...
export function Sidebar() {
        ...
        if (currentWorkspaceId) {
          ...
          if (matchingWorkspace) {
            ...
            if (fetchedWorkspaces.length > 0) {
              ...
              currentRouter?.push(`/arena/${fallbackWorkspace.id}/zelaxy`)
            } else {
  ...
  const updateWorkspaceName = useCallback(
    async (workspaceId: string, newName: string): Promise<boolean> => {
      try {
        const response = await fetch(`/api/arenas/${workspaceId}`, {
          method: 'PATCH',
  ...
  const switchWorkspace = useCallback(
    async (workspace: Workspace) => {
      ...
      try {
        ...
        routerRef.current?.push(`/arena/${workspace.id}/zelaxy`)
  ...
  const confirmDeleteWorkspace = useCallback(
    async (workspaceToDelete: Workspace) => {
      ...
      try {
        ...
        const response = await fetch(`/api/arenas/${workspaceToDelete.id}`, {
          method: 'DELETE',
        })
  ...
  const handleLeaveWorkspace = useCallback(
    async (workspaceToLeave: Workspace) => {
      ...
      try {
        ...
        const response = await fetch(`/api/arenas/members/${sessionData?.user?.id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const searchWorkspaces = useMemo(() => {
    return workspaces.map((workspace) => ({
      id: workspace.id,
      name: workspace.name,
      href: `/arena/${workspace.id}/zelaxy`,
        ...
        >
          ...
          >
            <button
              ...
              className='flex h-10 w-full cursor-pointer items-center gap-2.5 rounded-xl border border-border/60 bg-card/50 pr-2.5 pl-3 shadow-sm backdrop-blur-sm transition-all hover:border-border hover:bg-accent/40'
            >
              <Search className='h-3.5 w-3.5 text-muted-foreground/70' strokeWidth={2} />
              <span className='flex h-8 flex-1 items-center px-0 text-[13px] text-muted-foreground/70 leading-none'>
                Search or navigate...
              </span>
          ...
          {/* 4. Workflow Selector */}
          <div
            className={`pointer-events-auto relative h-[212px] flex-shrink-0 overflow-hidden rounded-xl border border-border/60 bg-card/50 shadow-sm backdrop-blur-sm ${
    ...
    >
      <span className='flex items-center justify-center gap-[1px]'>
        {parts.map((part, index) => (
          <span key={index} className={cn(isSymbol(part) ? 'text-[13px]' : 'text-[10px]')}>
            {part}
          </span>
        ))}
      </span>
...
const NavigationItem = ({ item }: NavigationItemProps) => {
  // Settings and help buttons get gray hover, others get blue hover
  const isGrayHover = item.id === 'settings' || item.id === 'help'

  const content = item.disabled ? (
    <div className='inline-flex h-9 w-9 cursor-not-allowed items-center justify-center rounded-lg text-muted-foreground/40 transition-colors [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0'>
    ...
    <Button
      ...
      className={cn(
        'h-9 w-9 rounded-lg text-muted-foreground transition-all duration-200',
        isGrayHover && 'hover:bg-muted/80 hover:text-foreground',
        !isGrayHover && 'hover:bg-primary/10 hover:text-primary',
        item.active &&
