...
export function AvatarPicker({ onAvatarUpdated }: AvatarPickerProps) {
  ...
  const handleTogglePicker = () => {
    if (isPickerOpen) {
      // Closing without saving — discard any in-progress selection.
      setSelectedAvatar(null)
      setSaveStatus({ type: null, message: '' })
    }
      ...
      <div className='flex items-center gap-4'>
        ...
        <div className='min-w-0 flex-1'>
          ...
          <p className='mt-0.5 text-[12px] text-muted-foreground'>
            {hasOAuthImage
              ? 'Using your sign-in provider avatar.'
              : 'This is the avatar other people see.'}
          </p>
        </div>
        <Button
          variant='outline'
          ...
          className='h-8 shrink-0 gap-1 rounded-lg px-3 text-[12px]'
        >
          {isPickerOpen ? 'Cancel' : 'Change avatar'}
          {!isPickerOpen && <ChevronRight className='h-3.5 w-3.5' />}
        </Button>
      </div>

      {/* Avatar Grid — only shown once the user asks to change the avatar */}
      {isPickerOpen && (
        <div className='space-y-2.5 border-border/40 border-t pt-4'>
