...
export function SearchModal({
  ...
  const blocks = useMemo(() => {
    ...
    // Add special blocks (loop and parallel) if not already present
    const existingTypes = new Set(regularBlocks.map((b) => b.type))
  ...
  // Quick navigation pages - shown prominently when no search query
  const quickNavPages = useMemo(
    () => [
      {
        id: 'knowledge',
        name: 'Knowledge',
  ...
  const navigationSections = useMemo((): NavigationSection[] => {
    ...
    // Quick Navigation section - only when no search query
    if (!searchQuery.trim() && quickNavPages.length > 0) {
      sections.push({
        id: 'quicknav',
        name: 'Quick Navigation',
        type: 'list',
        items: quickNavPages.map((item) => ({ type: 'quicknav', data: item })),
      })
    }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (isModifierPressed && e.shiftKey) {
        ...
        if (e.key.toLowerCase() === 'l') {
          e.preventDefault()
          handlePageClick(`/arena/${workspaceId}/hub?tab=logs`)
        }
        // Command+Shift+K - Navigate to Knowledge
        else if (e.key.toLowerCase() === 'k') {
          e.preventDefault()
          handlePageClick(`/arena/${workspaceId}/hub?tab=knowledge`)
        }
      }
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPortal>
        ...
        />
        <DialogPrimitive.Content className='data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] fixed top-[50%] left-[50%] z-50 flex h-[580px] w-[700px] translate-x-[-50%] translate-y-[-50%] flex-col gap-0 overflow-hidden border border-border bg-background p-0 focus:outline-none focus-visible:outline-none data-[state=closed]:animate-out data-[state=open]:animate-in'>
          <VisuallyHidden.Root>
          ...
          >
            <div className='space-y-6 pt-6 pb-6'>
              {/* Quick Navigation Section - Only when no search query */}
              {!searchQuery.trim() && quickNavPages.length > 0 && (
                <div
                  ref={(el) => {
                    if (el) scrollRefs.current.set('quicknav', el)
                  }}
                >
                  ...
                  <div className='space-y-0.5 px-6'>
                    {quickNavPages.map((page, index) => (
                      <button
                        key={page.id}
                        onClick={() => handlePageClick(page.href)}
                        data-nav-item={`quicknav-${index}`}
                        className={`group flex h-14 w-full items-center gap-4 rounded-lg px-3 py-2 transition-colors focus:outline-none ${
                          isItemSelected('quicknav', index)
                      ...
                      >
                        ...
                        <div className='flex flex-1 flex-col items-start gap-0.5'>
                          ...
                          <span className='text-muted-foreground text-xs'>{page.description}</span>
                        </div>
                  ...
                  >
                    {filteredBlocks.map((block, index) => (
                      <button
                        ...
                        className={`flex h-auto w-[180px] flex-shrink-0 cursor-pointer flex-col items-start gap-2 border p-3 transition-all duration-200 ${
                        ...
                        {filteredWorkspaces.map((workspace, workspaceIndex) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex h-10 w-full items-center gap-3 px-3 py-2 transition-colors focus:outline-none ${
...
const KeyboardShortcut = ({ shortcut, className }: KeyboardShortcutProps) => {
  ...
  return (
    <kbd
      className={cn(
        'flex h-6 w-9 items-center justify-center border border-border bg-background font-mono text-muted-foreground text-xs dark:text-muted-foreground',
        className
      )}
    >
