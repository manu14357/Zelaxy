...
function transformBlockData(data: any, blockType: string, isInput: boolean) {
  ...
  if (isInput) {
    ...
    // For agent blocks, surface file processing metadata clearly
    if (blockType === 'agent' && cleanInput._filesProcessed) {
      const filesProcessed = cleanInput._filesProcessed
      // Move _filesProcessed to a user-friendly key and format it
      cleanInput['Attached Files'] = filesProcessed.map((f: any) => ({
        name: f.name,
        size: f.size ? `${(f.size / 1024).toFixed(1)} KB` : 'unknown',
        type: f.type || 'unknown',
        parsedContent: f.parsedContent || 'No content extracted',
      }))
      cleanInput._filesProcessed = undefined
    }

    // For agent blocks with very long userPrompt (contains file content),
    // show a truncated version with a note about attached file content
    if (blockType === 'agent' && typeof cleanInput.userPrompt === 'string') {
      const fileMarker = '[ATTACHED FILES]'
      const markerIndex = cleanInput.userPrompt.indexOf(fileMarker)
      if (markerIndex >= 0) {
        const originalPrompt = cleanInput.userPrompt.substring(0, markerIndex).trim()
        const fileSection = cleanInput.userPrompt.substring(markerIndex)
        // Count both regular file entries and OCR-processed entries
        const regularCount = (fileSection.match(/--- Attached File:/g) || []).length
        const ocrCount = (fileSection.match(/--- OCR Extracted Text:/g) || []).length
        const fileCount = regularCount + ocrCount
        cleanInput.userPrompt = originalPrompt
        const label =
          ocrCount > 0
            ? `${fileCount} file(s) content included in prompt (${ocrCount} via OCR, ${fileSection.length.toLocaleString()} chars)`
            : `${fileCount} file(s) content included in prompt (${fileSection.length.toLocaleString()} chars)`
        cleanInput['File Content Appended'] = label
      }
    }
...
function CollapsibleInputOutput({ span, spanId }: CollapsibleInputOutputProps) {
  ...
  return (
    <div className='mt-2 mr-4 mb-3 ml-8 space-y-2 overflow-hidden'>
      {/* Input Data - Collapsible */}
      {span.input && (
        <div>
          <button
            onClick={() => setInputExpanded(!inputExpanded)}
            className='mb-1.5 flex items-center gap-1.5 font-medium text-[11px] text-muted-foreground uppercase tracking-wider transition-colors hover:text-foreground'
          >
            {inputExpanded ? (
          ...
          {inputExpanded && (
            <div className='mb-2 overflow-hidden rounded-lg bg-muted/20 p-2.5'>
...
function BlockDataDisplay({
  ...
  const renderValue = (value: any, key?: string): React.ReactNode => {
    ...
    if (typeof value === 'number') {
      return <span className='text-primary dark:text-primary/80'>{value}</span>
    }

    if (typeof value === 'boolean') {
      return <span className='text-primary dark:text-primary/80'>{value.toString()}</span>
    }
...
function TraceSpanItem({
  ...
  const getSpanIcon = () => {
    ...
    if (type === 'agent') {
      return <AgentIcon className='h-3 w-3 text-primary' />
    }

    if (type === 'evaluator') {
      return <ChartBarIcon className='h-3 w-3 text-primary' />
    }
    ...
    if (type === 'router') {
      return <ConnectIcon className='h-3 w-3 text-primary' />
    }
    ...
    if (type === 'tool') {
      return <ExternalLink className='h-3 w-3 text-primary' />
    }

    if (type === 'api') {
      return <ApiIcon className='h-3 w-3 text-primary' />
    }
  ...
  const getSpanColor = (type: string) => {
    switch (type.toLowerCase()) {
      case 'agent':
        return '#f97316' // Primary orange from AgentBlock
      case 'provider':
      ...
      case 'api':
        return '#EA580C' // Blue from ApiBlock
      default:
      ...
      >
        ...
        <div className='flex min-w-0 flex-1 items-center gap-2 overflow-hidden'>
          ...
          <div className='ml-auto flex w-[40%] flex-shrink-0 items-center gap-2'>
            ...
            <div className='relative hidden h-1.5 min-w-[15%] flex-1 flex-shrink-0 overflow-hidden rounded-full bg-muted/40 sm:block'>
              <div
            ...
            <span className='w-[65px] flex-shrink-0 text-right font-mono text-[11px] text-muted-foreground/60 tabular-nums'>
              {`${duration}ms`}
            </span>
          </div>
        </div>
