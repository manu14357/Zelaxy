  ...
  useEffect(() => {
    ...
    try {
      ...
      mat = new THREE.MeshBasicMaterial({
        color: 0x3b82f6, // Light blue color
        wireframe: true,
      })
    }
    ...
    const updateShaderColors = (
      ...
      if (isMuted) {
        ...
        // Interruption: bright blue
        uniforms.u_red.value = 0.15
        uniforms.u_green.value = 0.39
        uniforms.u_blue.value = 0.93
      } else if (isPlayingAudio) {
        // AI speaking: brand orange (#F97316)
        uniforms.u_red.value = 0.23
        uniforms.u_green.value = 0.51
        uniforms.u_blue.value = 0.96
      } else if (isListening && avgLevel > 10) {
        // User speaking: lighter blue with intensity-based variation
        const intensity = Math.min(avgLevel / 50, 1)
        uniforms.u_red.value = 0.38 + intensity * 0.15
        uniforms.u_green.value = 0.65 + intensity * 0.1
        uniforms.u_blue.value = 0.98 + intensity * 0.02
      } else if (isStreaming) {
        // AI thinking: pulsing brand blue
        const pulse = (Math.sin(elapsedTime * 2) + 1) / 2
        uniforms.u_red.value = 0.23 + pulse * 0.1
        uniforms.u_green.value = 0.51 + pulse * 0.1
        uniforms.u_blue.value = 0.96 + pulse * 0.02
      } else {
        // Default idle: soft brand blue
        uniforms.u_red.value = 0.23
        uniforms.u_green.value = 0.51
        uniforms.u_blue.value = 0.96
      }
