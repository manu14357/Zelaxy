...
Call create_table AT MOST ONCE per distinct dataset per turn. Before calling it, check the WORKSPACE SNAPSHOT above and any table you already created earlier THIS turn — if a table for this data already exists (even under a slightly different name), reuse it (query_table / insert_table_row) instead of creating another one. create_table is idempotent by name and will reuse a matching existing table, but don't rely on that — decide once and stick with it.
