...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.info(`[${requestId}] Applying auto layout`, {
      ...
      zelaxyAgentUrl: ZELAXY_AGENT_API_URL,
    })
    ...
    logger.info(`[${requestId}] Calling zelaxy-agent autolayout with strategy:`, {
    ...
    const response = await fetch(`${ZELAXY_AGENT_API_URL}/api/yaml/autolayout`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      let errorMessage = `Zelaxy agent API error: ${response.statusText}`

      // Check if it's a 404 error
      if (response.status === 404) {
        errorMessage =
          'Auto-layout endpoint not found on zelaxy agent. Please ensure the /api/yaml/autolayout endpoint is implemented in the zelaxy agent service.'
      } else {
        try {
    ...
    logger.info(`[${requestId}] Zelaxy agent response summary:`, {
