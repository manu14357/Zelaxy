...
import { db } from '@/db'
import { member, permissions, user, workflow, workflowBlocks, workspace } from '@/db/schema'
...
export async function GET() {
  ...
  // Verify the user still exists in DB (handles ghost sessions from deleted users)
  ...
  if (existingUser.length === 0) {
    logger.warn('Ghost session detected — user no longer exists', { userId: session.user.id })
  ...
  // Also get workspaces linked to organizations the user belongs to
  ...
  let orgWorkspaces: typeof userWorkspaces = []
  if (orgIds.length > 0) {
    const orgLinkedWorkspaces = await db
      .select({ workspace: workspace })
      .from(workspace)
      .where(inArray(workspace.organizationId, orgIds))
      .orderBy(desc(workspace.createdAt))

    // Convert to same shape, assigning 'read' as default permission for org members
    const existingIds = new Set(userWorkspaces.map((w) => w.workspace.id))
    orgWorkspaces = orgLinkedWorkspaces
      .filter((w) => !existingIds.has(w.workspace.id))
      .map((w) => ({
  ...
  const allWorkspaces = [...userWorkspaces, ...orgWorkspaces]
  ...
  const ownedWorkspace = userWorkspaces.length > 0 ? userWorkspaces[0].workspace.id : null
  if (ownedWorkspace) {
    await ensureWorkflowsHaveWorkspace(session.user.id, ownedWorkspace)
  }

  // Format the response with permission information
  const workspacesWithPermissions = allWorkspaces.map(
...
// POST /api/arenas - Create a new workspace
