...
import { db } from '@/db'
import { knowledgeBase, member, permissions, workspace } from '@/db/schema'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Check if user has any access to this workspace (includes org membership check)
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Check if user has admin permissions to update workspace (includes org membership check)
  const userPermission = await getUserEntityPermissions(session.user.id, 'workspace', workspaceId)
  ...
  try {
    ...
    if (!name && organizationId === undefined) {
      return NextResponse.json({ error: 'Name or organizationId is required' }, { status: 400 })
    }

    // If linking to an org, verify user is a member of that org
    if (organizationId) {
