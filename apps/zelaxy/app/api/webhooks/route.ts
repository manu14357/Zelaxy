...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existingWebhooks.length > 0 && existingWebhooks[0].workflowId === workflowId) {
      ...
      // Preserve critical fields (userId, historyId, etc.) from the existing providerConfig
      // so that polling services can still authenticate. The configureGmailPolling /
      // configureOutlookPolling functions will re-set these, but there is a race window
      // where the polling job can read the webhook before the configure step completes.
      const existingWebhookData = await db
        .select({ providerConfig: webhook.providerConfig })
        .from(webhook)
        .where(eq(webhook.id, existingWebhooks[0].id))
        .limit(1)

      const existingConfig = (existingWebhookData[0]?.providerConfig as Record<string, any>) || {}
      const mergedProviderConfig = {
        ...existingConfig,
        ...(finalProviderConfig as Record<string, any>),
        // Always preserve userId from existing config if the incoming config doesn't have one
        ...(existingConfig.userId && !(finalProviderConfig as any)?.userId
