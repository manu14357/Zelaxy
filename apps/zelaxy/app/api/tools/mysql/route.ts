...
import { MySQLDatabase } from '@/tools/mysql/database'
import type { MySQLQueryParams, MySQLResponse } from '@/tools/mysql/types'
import { validateConnectionParams, validateQuery } from '@/tools/mysql/utils'

function createConnectionConfig(params: MySQLQueryParams) {
  return {
    host: params.host,
    database: params.database,
    user: params.username,
    password: params.password || '',
    port: params.port ? Number.parseInt(params.port.toString(), 10) : 3306,
    ssl: params.ssl,
  }
}
...
export async function POST(request: NextRequest) {
  try {
    const params: MySQLQueryParams = await request.json()
...
async function handleExecuteQuery(params: MySQLQueryParams): Promise<MySQLResponse> {
  if (!params.query) {
    return {
      success: true,
      output: {
  ...
  return await MySQLDatabase.executeQuery(config, params.query)
}

async function handleInsert(params: MySQLQueryParams): Promise<MySQLResponse> {
  ...
  const config = createConnectionConfig(params)
  return await MySQLDatabase.insertData(config, params.table, params.data)
}

async function handleUpdate(params: MySQLQueryParams): Promise<MySQLResponse> {
  ...
  const config = createConnectionConfig(params)
  return await MySQLDatabase.updateData(config, params.table, params.data, params.conditions)
}

async function handleDelete(params: MySQLQueryParams): Promise<MySQLResponse> {
  ...
  const config = createConnectionConfig(params)
  return await MySQLDatabase.deleteData(config, params.table, params.conditions)
}
