...
describe('Copilot Chat API Route', () => {
  ...
  beforeEach(() => {
    ...
    vi.doMock('@/lib/env', () => ({
      env: {
        ZELAXY_AGENT_API_URL: 'http://localhost:8000',
        ZELAXY_AGENT_API_KEY: 'test-zelaxy-agent-key',
      },
    }))
  ...
  describe('POST', () => {
    ...
    it('should handle new chat creation and forward to zelaxy agent', async () => {
      const authMocks = mockAuth()
      authMocks.setAuthenticated()
      ...
      // Mock successful zelaxy agent response
      ...
      // Verify zelaxy agent was called
      expect(global.fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          headers: {
            ...
            'x-api-key': 'test-zelaxy-agent-key',
          },
          body: JSON.stringify({
            messages: [
              {
    ...
    it('should handle zelaxy agent API errors', async () => {
      const authMocks = mockAuth()
      authMocks.setAuthenticated()
      ...
      const responseData = await response.json()
      expect(responseData.error).toContain('Zelaxy agent API error')
    })
