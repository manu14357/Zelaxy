...
const ChatMessageSchema = z.object({
  ...
  provider: z.string().optional(), // Allow override of LLM provider
  model: z.string().optional(), // Allow override of LLM model
  customApiKey: z.string().optional(), // Custom API key for non-default providers
})

// Zelaxy Agent API configuration - disabled to force direct provider usage
const ZELAXY_AGENT_API_URL = env.ZELAXY_AGENT_API_URL // Remove fallback to disable agent API
 ...
 * Send messages to zelaxy agent and handle chat persistence
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (chatId) {
      ...
      // Create new chat - use requested provider/model or fall back to default
      const { provider, model } =
        requestProvider && requestModel
          ? { provider: requestProvider, model: requestModel }
          : getCopilotModel('chat')
    ...
    // Build messages array for zelaxy agent with conversation history
    const messages = []
    ...
    logger.info(`[${tracker.requestId}] Using LLM provider`, {
      provider,
      model,
      messageCount: messages.length,
      isUserSelected: !!(requestProvider && requestModel),
    })

    // Check if agent API is available, otherwise use direct provider
    if (!ZELAXY_AGENT_API_URL) {
      logger.info(
        `[${tracker.requestId}] Agent API not available, redirecting to direct provider API with local tools`
      )

      // Redirect to direct provider API with proper authentication
      const directApiUrl = `/api/copilot/direct-chat`
      const directResponse = await fetch(`${req.nextUrl.origin}${directApiUrl}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // Forward the original request headers including cookies for authentication
        ...
        body: JSON.stringify({
          ...
          chatId: actualChatId,
          workflowId,
          mode, // Pass through the mode to enable tools in agent mode
    ...
    // Forward to zelaxy agent API
    logger.info(`[${tracker.requestId}] Sending request to zelaxy agent API`, {
      messageCount: messages.length,
      endpoint: `${ZELAXY_AGENT_API_URL}/api/chat-completion-streaming`,
    })

    const zelaxyAgentResponse = await fetch(
      `${ZELAXY_AGENT_API_URL}/api/chat-completion-streaming`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          // Add provider and model selection if specified
    ...
    if (stream && zelaxyAgentResponse.body) {
      ...
      const transformedStream = new ReadableStream({
        async start(controller) {
          ...
          // Forward the zelaxy agent stream and capture assistant response
          const reader = zelaxyAgentResponse.body!.getReader()
          ...
          try {
            while (true) {
              ...
              try {
                ...
                // Client disconnected - stop reading from zelaxy agent
                logger.info(
                  `[${tracker.requestId}] Client disconnected, stopping stream processing`
                )
                reader.cancel() // Stop reading from zelaxy agent
                break
              }
              ...
              for (const line of lines) {
                ...
                if (line.startsWith('data: ') && line.length > 6) {
                  ...
                  logger.debug(`[${tracker.requestId}] Non-SSE line from zelaxy agent: "${line}"`)
                }
              }
            }
    ...
    const responseData = await zelaxyAgentResponse.json()
    logger.info(`[${tracker.requestId}] Non-streaming response from zelaxy agent:`, {
