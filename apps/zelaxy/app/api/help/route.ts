...
${message}
    ...
    // Get email domain and ensure it's valid for production
    const emailDomain = getEmailDomain()
    const fromEmail = emailDomain.includes('localhost')
      ? 'noreply@resend.dev'
      : `noreply@${emailDomain}`
    const toEmail = emailDomain.includes('localhost')
      ? 'delivered@resend.dev'
      : `help@${emailDomain}`

    logger.debug(`[${requestId}] Email configuration`, {
      emailDomain,
      fromEmail,
      toEmail,
    })
    ...
    await resend.emails
      .send({
        from: `Zelaxy Support <${fromEmail}>`,
        to: [email],
        subject: `Your ${typeLabel.toLowerCase()} request has been received: ${subject}`,
        text: `
Hello,

Thank you for your ${typeLabel.toLowerCase()} submission. We've received your request and will get back to you as soon as possible.
