...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isUsingCloudStorage()) {
      logger.info(`Local storage detected, presigned URLs not supported for ${fileName}`)
      return NextResponse.json(
        {
          directUploadSupported: false,
          message:
            'Direct uploads are not available for local storage. Use /api/files/upload instead.',
          fallbackUploadUrl: '/api/files/upload',
        },
        { status: 200 }
      )
    }
...
async function handleS3PresignedUrl(
  ...
  try {
    ...
    // Properly sanitize filename using the same logic as local client
    // Generate a unique key with timestamp prefix to avoid collisions
    const timestamp = Date.now()
    const shortUuid = uuidv4().split('-')[0] // Short UUID like local client
    const extension = fileName.includes('.') ? fileName.split('.').pop() : ''
    const baseName = fileName.replace(/\.[^/.]+$/, '') // Remove extension

    // Sanitize the base name to remove control characters and problematic characters
    const sanitizedBaseName = baseName
      .replace(/[\x00-\x1F\x7F]/g, '') // Remove control characters like ^M
    ...
    const safeFileName = extension
      ? `${timestamp}-${shortUuid}-${sanitizedBaseName}.${extension}`
      : `${timestamp}-${shortUuid}-${sanitizedBaseName}`
    ...
    const uniqueKey = `${prefix}${safeFileName}`
