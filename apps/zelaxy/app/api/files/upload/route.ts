  ...
  try {
    ...
    for (const file of files) {
      ...
      if (workflowId && executionId) {
        ...
        // Parse file content for text-based files so the agent can read them
        try {
            ...
            if (parseResult.content) {
              ...
              userFile.parsedContent =
                parseResult.content.length > MAX_CONTENT
                  ...
                  : parseResult.content
              logger.info(
                `Parsed file content for ${originalName}: ${userFile.parsedContent.length} chars`
              )
            }
          }
        } catch (parseError) {
          logger.warn(`Failed to parse file content for ${originalName}:`, parseError)
        }
