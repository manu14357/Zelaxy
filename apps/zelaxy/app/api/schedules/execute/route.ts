...
// Register MCPService so the executor's AgentBlockHandler can access it
...
function calculateNextRunTime(
  ...
  if (schedule.cronExpression) {
    const cron = new Cron(schedule.cronExpression, { timezone: schedule.timezone || 'UTC' })
...
export async function GET() {
  logger.debug(`Scheduled execution triggered at ${new Date().toISOString()}`)
                      ...
                      if (
                        ...
                        if (matches) {
                          for (const match of matches) {
                            ...
                            // Skip block references (containing dots like start.hello, block.output)
                            // and workflow variable references (starting with variable.)
                            if (varName.includes('.')) {
                              continue
                            }
