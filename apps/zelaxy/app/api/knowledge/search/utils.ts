...
/**
 * Generate search embedding using Ollama API
 */
async function generateOllamaSearchEmbedding(
  query: string,
  ...
  try {
    ...
    logger.error('Failed to generate search embedding with Ollama:', error)
    throw new Error(
...
export async function generateSearchEmbedding(
  ...
  const isOllamaModel =
    ...
    embeddingModel.includes('ollama') ||
  ...
  if (isOllamaModel) {
    rawEmbedding = await generateOllamaSearchEmbedding(query, embeddingModel)
  } else {
    const openaiApiKey = env.OPENAI_API_KEY
    if (!openaiApiKey) {
      throw new Error(
        'OPENAI_API_KEY not configured. For local search, use Ollama with models like "nomic-embed-text"'
      )
    }

    try {
      rawEmbedding = await retryWithExponentialBackoff(
        async () => {
  ...
  const normalizedEmbedding = normalizeEmbeddingDimensions(rawEmbedding, 2000)

  logger.debug(
    `Generated search embedding: ${rawEmbedding.length} dims -> ${normalizedEmbedding.length} dims`
  )
 ...
 * Pads shorter vectors with zeros or truncates longer vectors
 */
