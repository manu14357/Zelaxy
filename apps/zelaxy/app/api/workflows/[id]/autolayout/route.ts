...
import { zelaxyAgentClient } from '@/lib/zelaxy-agent'
...
const ZELAXY_AGENT_ENABLED = !!ZELAXY_AGENT_API_KEY
if (!ZELAXY_AGENT_ENABLED) {
  logger.warn('ZELAXY_AGENT_API_KEY not configured - autolayout requests will fail')
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Check if zelaxy-agent service is available
    if (!ZELAXY_AGENT_ENABLED) {
      logger.warn(
        `[${requestId}] Zelaxy-agent service not available - returning current workflow without autolayout`
      )
      return NextResponse.json({
        success: true,
        message: 'Autolayout service not available, returning current workflow positions',
    ...
    logger.info(
      `[${requestId}] Applying autolayout to ${Object.keys(currentWorkflowData.blocks).length} blocks`,
      {
        ...
        zelaxyAgentUrl: process.env.ZELAXY_AGENT_API_URL || 'http://localhost:8000',
      }
    )
    ...
    // Gather block registry and utilities for zelaxy-agent
    ...
    const autoLayoutResult = await zelaxyAgentClient.makeRequest('/api/yaml/autolayout', {
      body: {
        workflowState,
      ...
      apiKey: ZELAXY_AGENT_API_KEY,
    })

    // Log the full response for debugging
    logger.info(`[${requestId}] Zelaxy-agent autolayout response:`, {
    ...
    if (
      ...
      (!autoLayoutResult.data?.workflowState &&
        !autoLayoutResult.data?.blocks &&
        !autoLayoutResult.data?.data?.workflowState)
    ) {
      ...
      const errorMessage =
        ...
        (autoLayoutResult.status === 401
          ...
          : autoLayoutResult.status === 404
            ? 'Zelaxy-agent service not found'
    ...
    // Handle both response formats from zelaxy-agent
    const layoutedBlocks =
      autoLayoutResult.data?.workflowState?.blocks ||
      autoLayoutResult.data?.blocks ||
      autoLayoutResult.data?.data?.workflowState?.blocks

    if (!layoutedBlocks) {
      logger.error(`[${requestId}] No blocks returned from zelaxy-agent:`, {
        responseData: autoLayoutResult.data,
      })
      return NextResponse.json(
        {
          error: 'Auto layout failed',
          details: 'No blocks returned from zelaxy-agent',
        },
        { status: 500 }
      )
    }
