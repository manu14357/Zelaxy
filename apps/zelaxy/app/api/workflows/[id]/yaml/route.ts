...
async function createWorkflowCheckpoint(
  ...
  try {
    ...
    if (currentWorkflowData) {
      ...
      const generateResponse = await fetch(`${ZELAXY_AGENT_API_URL}/api/workflow/to-yaml`, {
        method: 'POST',
        headers: {
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Convert YAML to workflow state by calling zelaxy-agent directly
    // Gather block registry and utilities for zelaxy-agent
    ...
    const conversionResponse = await fetch(`${ZELAXY_AGENT_API_URL}/api/yaml/to-workflow`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(ZELAXY_AGENT_API_KEY && { 'x-api-key': ZELAXY_AGENT_API_KEY }),
    ...
    if (!conversionResponse.ok) {
      ...
      return NextResponse.json({
        ...
        errors: [`Zelaxy agent API error: ${conversionResponse.statusText}`],
        warnings: [],
      })
    }
