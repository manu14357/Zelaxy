  ...
  devtools(
    (set, get) => ({
      ...
      loadOrganizationSubscription: async (orgId: string) => {
        // Skip subscription loading when billing is disabled
        if (!isBillingEnabled) {
          set({
            subscriptionData: null,
            isLoadingSubscription: false,
      ...
      loadOrganizationBillingData: async (organizationId: string) => {
        // Skip billing data fetch entirely when billing is disabled
        if (!isBillingEnabled) {
          set({
      ...
      loadUserWorkspaces: async (userId?: string) => {
        try {
          ...
          const workspacesResponse = await fetch('/api/arenas')
          if (!workspacesResponse.ok) {
            logger.error('Failed to fetch arenas')
            return
          }
          ...
          for (const workspace of allUserWorkspaces) {
            try {
              const permissionResponse = await fetch(`/api/arenas/${workspace.id}/permissions`)
      ...
      refreshOrganization: async () => {
        ...
        try {
          ...
          // Clear cache so loadData won't overwrite with stale data
          set({
          ...
          if (updatedOrg?.id) {
            set({ lastSubscriptionFetched: null })
      ...
      createOrganization: async (name: string, slug: string) => {
        ...
        try {
          ...
          // Clear cache and refresh data so the new org shows up immediately
          set({ lastFetched: null, lastSubscriptionFetched: null })
          await get().loadData()
      ...
      inviteMember: async (email: string, workspaceInvitations?: WorkspaceInvitation[]) => {
        ...
        try {
          // Only enforce seat limits when billing is enabled
          if (isBillingEnabled) {
