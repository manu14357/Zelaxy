  ...
  devtools(
    (set, get) => ({
      ...
      sendMessage: async (message: string, options = {}) => {
        ...
        try {
          // Get current LLM selection - import here to avoid circular dependencies
          const { useLLMSelectionStore } = await import('@/stores/llm-selection/store')
          const llmSelection = useLLMSelectionStore.getState()

          // Debug log the current selection
          logger.info('Current LLM selection state:', {
            selectedProvider: llmSelection.selectedProvider,
            selectedModel: llmSelection.selectedModel,
          })

          // Check if user has selected a custom provider/model (not the default system)
          ...
          const shouldUseDirectChat = mode === 'ask' && isCustomSelection

          logger.info('Provider selection decision:', {
            mode,
            isCustomSelection,
            shouldUseDirectChat,
            selectedProvider: llmSelection.selectedProvider,
            selectedModel: llmSelection.selectedModel,
            willUseDirectAPI: shouldUseDirectChat,
          })
          ...
          if (shouldUseDirectChat && llmSelection.selectedProvider && llmSelection.selectedModel) {
            ...
            logger.info('Using direct provider API for Ask mode with custom selection:', {
            ...
            result = await sendDirectMessage({
              ...
              workflowId,
              mode: 'ask', // Explicitly set Ask mode - no tools needed
            ...
            result = await sendDirectMessage({
              ...
              chatId: currentChat?.id,
              workflowId,
              mode: 'agent', // Agent mode - includes workflow tools
              createNewChat: !currentChat,
              stream,
              fileAttachments: options.fileAttachments,
              abortSignal: abortController.signal,
            ...
            // Use agent API for:
            // 1. Agent mode (always - needs tools for workflow creation/editing)
            // 2. Ask mode with default provider
            // This ensures workflow tools are available when needed
            const customApiKey = isCustomSelection
              ? llmSelection.getApiKey?.(llmSelection.selectedProvider)
              : undefined

            logger.info('Using agent API with tools:', {
              mode,
              provider: llmSelection.selectedProvider,
              model: llmSelection.selectedModel,
              hasCustomApiKey: !!customApiKey,
            ...
            result = await sendStreamingMessage({
              ...
              provider: llmSelection.selectedProvider,
              model: llmSelection.selectedModel,
              customApiKey, // Pass custom API key for non-default providers
            })
          }
        ...
        try {
          ...
          if (isCustomSelection && llmSelection.selectedProvider && llmSelection.selectedModel) {
            ...
            result = await sendDirectMessage({
              ...
              provider: llmSelection.selectedProvider,
              model: llmSelection.selectedModel,
              customApiKey, // Pass the custom API key if available
            })
          } else {
            // Use agent API for default system
            result = await sendStreamingMessage({
              message: 'Please continue your response.',
              chatId: currentChat?.id,
              workflowId,
      ...
      updateDiffStore: async (yamlContent: string, toolName?: string) => {
        ...
        try {
          ...
          // Skip diff analysis - let zelaxy-agent handle it through /api/yaml/diff/create
