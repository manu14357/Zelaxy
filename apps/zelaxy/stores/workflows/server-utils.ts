...
export function mergeSubblockState(
  ...
  return Object.entries(blocksToProcess).reduce(
    (acc, [id, block]) => {
      ...
      // For loop/parallel blocks, sync subblock values to block.data so the executor can read them
      // The executor reads from block.data (serialized as block.config.params) not block.subBlocks
      if (block.type === 'loop' || block.type === 'parallel') {
        ...
        const syncedData: Record<string, any> = { ...block.data }

        for (const field of subflowFields) {
          // Get value from provided values
          const subblockValue = blockValues[field]
          // Also check the merged subBlocks
          const subBlockState = mergedSubBlocks[field]
          const storedValue = subblockValue ?? subBlockState?.value

          if (storedValue !== undefined && storedValue !== null) {
            syncedData[field] = storedValue
          }
        }
