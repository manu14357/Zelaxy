  ...
  devtools(
    withHistory((set, get) => ({
      ...
      removeBlock: (id: string) => {
        ...
        set(newState)
        // Push history BEFORE cleaning up subblock values so that pushHistory
        // can capture the current subblock values (needed for undo to restore them)
        pushHistory(set, get, newState, 'Remove block and children')

        // Clean up subblock values AFTER history capture
      ...
      // Generic function to update arbitrary block.data fields (for loop/parallel properties)
      updateBlockData: (blockId: string, data: Record<string, any>) =>
        set((state) => {
          ...
          // Regenerate loops/parallels if applicable
          ...
          if (block.type === 'loop') {
            result.loops = generateLoopBlocks(newBlocks)
          } else if (block.type === 'parallel') {
            result.parallels = generateParallelBlocks(newBlocks)
          }
        ...
        set((state) => {
          ...
          const newBlocks = {
            ...
            [loopId]: {
              ...
              data: {
                ...block.data,
                count: Math.max(1, count), // Minimum 1 iteration
              },
            },
          }
      ...
      updateParallelCount: (parallelId: string, count: number) => {
        ...
        const newBlocks = {
          ...
          [parallelId]: {
            ...
            data: {
              ...block.data,
              count: Math.max(1, count), // Minimum 1 parallel execution
            },
          },
        }
