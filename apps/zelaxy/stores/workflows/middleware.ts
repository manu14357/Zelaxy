...
export const pushHistory = (
  ...
  // Refresh the outgoing present's subblock values with the latest from the store.
  // This captures any property edits made between the last pushHistory call and now
  // (e.g. user typed into fields without triggering a history push).
  const activeWorkflowId = useWorkflowRegistry.getState().activeWorkflowId
  let updatedPresent = history.present
  if (activeWorkflowId) {
    const currentValues = useSubBlockStore.getState().workflowValues[activeWorkflowId] || {}
    updatedPresent = {
      ...history.present,
  ...
  set({
    history: {
      past: [...history.past, updatedPresent].slice(-MAX_HISTORY_LENGTH),
      present: newEntry,
      future: [],
    },
    lastSaved: Date.now(),
