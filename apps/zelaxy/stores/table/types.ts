...
import type { ColumnDefinition, RowData } from '@/lib/table'
...
export type TableUndoAction =
  ...
  | { type: 'clear-cells'; cells: Array<{ rowId: string; data: RowData }> }
  | {
      type: 'update-cells'
      cells: Array<{
        rowId: string
  ...
  | { type: 'create-row'; rowId: string; position: number; data?: RowData }
  | {
      type: 'create-rows'
      rows: Array<{ rowId: string; position: number; data: RowData }>
    }
