...
/**
 * Shared execution core for Google's Gemini generative API.
 *
 * The public Gemini API (generativelanguage.googleapis.com, key in the query string) and Vertex AI
 * (`{location}-aiplatform.googleapis.com`, Bearer token) speak the *same* request/response shape —
 * only the URL and auth differ. Callers supply a `buildUrl` function and `headers` via `opts`, so
 * this function stays transport-agnostic while the tool loop, streaming, and structured-output
 * behavior are shared byte-for-byte.
 */
export type GoogleMethod = 'generateContent' | 'streamGenerateContent'

export interface GoogleCoreOptions {
  /** Build the request URL for a given model + generation method. */
  buildUrl: (model: string, method: GoogleMethod) => string
  /** Request headers (Content-Type, and for Vertex an `Authorization: Bearer …`). */
  headers: Record<string, string>
  /** Provider id used for forced-tool preparation/tracking (defaults to 'google'). */
  providerId?: string
}
...
export async function executeGoogleRequest(
  ...
  opts: GoogleCoreOptions
): Promise<ProviderResponse | StreamingExecution> {
  const providerId = opts.providerId || 'google'
  const { buildUrl, headers } = opts
  ...
  try {
    ...
    const endpoint = buildUrl(
      requestedModel,
      shouldStream ? 'streamGenerateContent' : 'generateContent'
    )
    ...
    try {
      ...
      // For copilot requests, DON'T auto-execute tools - return them for the caller to handle
      // Copilot has its own tool registry separate from workflow block tools
      if (request.isCopilotRequest && functionCall) {
        logger.info(`Copilot request - returning function call without auto-execution`)

        const copilotToolCalls = [
          {
          ...
          try {
            ...
            const simplifiedMessages = [
              ...
              // Function response - but use USER role since Gemini only accepts user or model
              {
                ...
                parts: [
                  {
                    text: `Function ${latestFunctionCall.name} result: ${toonEncodeForLLM(resultContent)}`,
                  },
                ],
              },
            ]
            ...
            try {
              ...
              if (request.stream) {
                ...
                const checkResponse = await fetch(buildUrl(requestedModel, 'generateContent'), {
                  method: 'POST',
                  headers,
                ...
                const streamingResponse = await fetch(
                  buildUrl(requestedModel, 'streamGenerateContent'),
                  {
                    method: 'POST',
                    headers,
              ...
              const nextResponse = await fetch(buildUrl(requestedModel, 'generateContent'), {
                method: 'POST',
                headers,
