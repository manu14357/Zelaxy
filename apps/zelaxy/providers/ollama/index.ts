  ...
  async initialize() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.debug('Ollama service is not available — provider disabled')
        return
      }
      ...
      logger.debug('Ollama not reachable — provider disabled', {
    ...
    try {
      ...
      if (request.isCopilotRequest) {
        ...
        if (toolCallsInResponse && toolCallsInResponse.length > 0) {
          logger.info(
            `Copilot request - returning ${toolCallsInResponse.length} tool calls without auto-execution`
          )
