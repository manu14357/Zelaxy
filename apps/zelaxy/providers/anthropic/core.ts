...
/**
 * Shared execution core for the Anthropic Messages API.
 *
 * Both the default Anthropic provider and the Azure-hosted Anthropic provider run the same request
 * pipeline (tool loops, forced-tool tracking, streaming, structured output). The only thing that
 * differs is how the `Anthropic` client is constructed (default api.anthropic.com vs. an
 * Azure-configured `baseURL` + `api-version`). Callers build the client and hand it in via `opts`,
 * so this function stays completely transport-agnostic.
 */
export interface AnthropicCoreOptions {
  /** A ready-to-use Anthropic SDK client (default or Azure-configured). */
  client: Anthropic
  /** Provider id used for forced-tool preparation/tracking (defaults to 'anthropic'). */
  providerId?: string
  /** Model id used when `request.model` is absent. */
  defaultModel?: string
}
...
export async function executeAnthropicRequest(
  ...
  opts: AnthropicCoreOptions
): Promise<ProviderResponse | StreamingExecution> {
  const anthropic = opts.client
  const providerId = opts.providerId || 'anthropic'
  const fallbackModel = opts.defaultModel || 'claude-3-7-sonnet-20250219'
    ...
    try {
      ...
      if (request.isCopilotRequest) {
        ...
        if (toolUses && toolUses.length > 0) {
          logger.info(
            `Copilot request - returning ${toolUses.length} tool calls without auto-execution`
          )

          // Format tool calls for the response
          const copilotToolCalls = toolUses.map((tc: any) => ({
            id: tc.id,
            name: tc.name,
            arguments: tc.input || {},
          }))
