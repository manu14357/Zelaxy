...
import { getToonSystemHint } from '@/lib/toon/encoder'
...
function sanitizeRequest(request: ProviderRequest): ProviderRequest {
  ...
  // Guard against negative or zero maxTokens — if the token budget is exhausted
  // or miscalculated, remove maxTokens entirely so the provider uses its default.
  if (sanitizedRequest.maxTokens !== undefined && sanitizedRequest.maxTokens < 1) {
    logger.warn(
      `maxTokens was ${sanitizedRequest.maxTokens}, which is invalid. Removing maxTokens to use provider default.`
    )
    sanitizedRequest.maxTokens = undefined
  }
...
export async function executeProviderRequest(
  ...
  // Append a short TOON format hint so the LLM knows data may arrive in TOON
  const toonHint = getToonSystemHint()
  if (toonHint) {
    sanitizedRequest.systemPrompt = (sanitizedRequest.systemPrompt || '') + toonHint
  }
