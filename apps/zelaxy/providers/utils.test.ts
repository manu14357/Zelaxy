...
describe('JSON and Structured Output', () => {
  ...
  describe('generateStructuredOutputInstructions', () => {
    it.concurrent('should generate instructions for JSON Schema format', () => {
      const schemaFormat = {
        schema: {
          type: 'object',
      ...
      const result = generateStructuredOutputInstructions(schemaFormat)
      expect(result).toContain('JSON')
      expect(result).toContain('key')
    })

    it.concurrent('should generate instructions for object type with properties', () => {
      const objectFormat = {
        type: 'object',
        properties: { key: { type: 'string' } },
      }
      const result = generateStructuredOutputInstructions(objectFormat)
      expect(result).toContain('JSON')
      expect(result).toContain('key')
    })
