...
> = {
  ...
  nvidia: {
    ...nvidiaProvider,
    models: getProviderModelsFromDefinitions('nvidia'),
    modelPatterns: PROVIDER_DEFINITIONS.nvidia.modelPatterns,
  },
  bedrock: {
    ...bedrockProvider,
...
// ── JSON Schema → system-prompt instructions ────────────────────────────────

/** Build a representative example value from a JSON Schema definition */
function buildExampleFromJsonSchema(schema: any): any {
  if (!schema) return {}

  if (schema.type === 'object' && schema.properties) {
    const example: Record<string, any> = {}
    for (const [key, prop] of Object.entries(schema.properties) as [string, any][]) {
      if (prop.type === 'string') example[key] = '<string>'
      else if (prop.type === 'integer') example[key] = 0
      else if (prop.type === 'number') example[key] = 0.0
      else if (prop.type === 'boolean') example[key] = true
      else if (prop.type === 'array') {
        if (prop.items) {
          example[key] = [buildExampleFromJsonSchema(prop.items)]
        } else {
          example[key] = []
        }
      } else if (prop.type === 'object') {
        example[key] = buildExampleFromJsonSchema(prop)
      } else {
        example[key] = null
      }
    }
...
/** Generate human-readable structured output instructions from a JSON Schema */
function generateJsonSchemaInstructions(schema: any, schemaName?: string): string {
  const example = buildExampleFromJsonSchema(schema)
  const exampleJson = isToonEnabled() ? toonEncodeForLLM(example) : JSON.stringify(example, null, 2)
  ...
  const fieldDescriptions = Object.entries(properties)
    .map(([key, prop]: [string, any]) => {
      const req = requiredFields.includes(key) ? 'required' : 'optional'
      const desc = prop.description ? `: ${prop.description}` : ''
      return `- ${key} (${prop.type}, ${req})${desc}`
    })
    .join('\n')

  logger.info(
    `Generated JSON-schema structured output instructions${schemaName ? ` for "${schemaName}"` : ''}`
  )

  return `You MUST respond with ONLY a valid JSON object matching the schema below. Do NOT include any text, explanation, or markdown before or after the JSON.
...
Your entire response must be a single valid JSON object containing all required fields with their correct types.`
}
...
export function generateStructuredOutputInstructions(responseFormat: any): string {
  ...
  // Handle JSON Schema format — generate instructions so that providers without
  // native structured-output support (e.g. Bedrock, DeepSeek, NVIDIA) still get
  // proper guidance via the system prompt. Providers that DO support it natively
  // (OpenAI, Anthropic, etc.) will also enforce it at the API level, so the extra
  // instructions are harmless reinforcement.
  if (responseFormat.schema || (responseFormat.type === 'object' && responseFormat.properties)) {
    ...
    return generateJsonSchemaInstructions(schema, responseFormat.name)
  }
