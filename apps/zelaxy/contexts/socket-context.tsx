...
interface SocketContextType {
  ...
  // Execution event handlers for real-time logs
  onExecutionStarted: (handler: (data: any) => void) => void
  onExecutionBlockComplete: (handler: (data: any) => void) => void
...
export function SocketProvider({ children, user }: SocketProviderProps) {
  ...
  const eventHandlers = useRef<{
    ...
    executionStarted?: (data: any) => void
    executionBlockComplete?: (data: any) => void
    executionComplete?: (data: any) => void
  }>({})
  ...
  useEffect(() => {
    ...
    const initializeSocket = async () => {
      try {
        ...
        // Execution events for real-time logs
        socketInstance.on('execution:started', (data) => {
          logger.info('Execution started:', data.executionId)
          eventHandlers.current.executionStarted?.(data)
        })

        socketInstance.on('execution:block-complete', (data) => {
          logger.debug('Execution block complete:', data.executionId)
          eventHandlers.current.executionBlockComplete?.(data)
        })

        socketInstance.on('execution:complete', (data) => {
          logger.info('Execution complete:', data.executionId)
          eventHandlers.current.executionComplete?.(data)
        })
  ...
  // Join workspace room (for execution events on logs page)
  const joinWorkspace = useCallback(
    (workspaceId: string) => {
      ...
      if (currentWorkspaceId === workspaceId) {
        logger.info(`Already in workspace ${workspaceId}, skipping join`)
        return
      }
    ...
    [socket, user, currentWorkspaceId]
  )

  // Leave current workspace room
  const leaveWorkspace = useCallback(() => {
    if (socket && currentWorkspaceId) {
      logger.info(`Leaving workspace: ${currentWorkspaceId}`)
  ...
  const onExecutionStarted = useCallback((handler: (data: any) => void) => {
    eventHandlers.current.executionStarted = handler
  }, [])

  const onExecutionBlockComplete = useCallback((handler: (data: any) => void) => {
    eventHandlers.current.executionBlockComplete = handler
  }, [])

  const onExecutionComplete = useCallback((handler: (data: any) => void) => {
    eventHandlers.current.executionComplete = handler
  }, [])
