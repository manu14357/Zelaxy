...
export async function validateSeatAvailability(
  ...
  try {
    // Skip seat validation entirely when billing is disabled — unlimited seats
    if (!isBillingEnabled) {
      // Get current member count for informational purposes
      ...
      return {
        ...
        currentSeats,
        maxSeats: 999,
        availableSeats: 999 - currentSeats,
      }
    }
