...
export class ExecutionLogger implements IExecutionLoggerService {
  ...
  /**
   * Append a single block's trace span to an in-progress execution log.
   * Uses a JSONB append operation so concurrent block completions are safe.
   * This enables real-time log streaming — each block's result appears
   * in the DB immediately rather than waiting for the full workflow to finish.
   */
  async appendBlockSpan(executionId: string, traceSpan: TraceSpan): Promise<void> {
    try {
      await db
        .update(workflowExecutionLogs)
        .set({
          metadata: sql`jsonb_set(
            COALESCE(${workflowExecutionLogs.metadata}, '{}'::jsonb),
            '{traceSpans}',
            COALESCE(${workflowExecutionLogs.metadata}->'traceSpans', '[]'::jsonb) || ${JSON.stringify(traceSpan)}::jsonb
          )`,
        })
        .where(eq(workflowExecutionLogs.executionId, executionId))

      logger.debug(`Appended block span for execution ${executionId}: ${traceSpan.name}`)
    } catch (error) {
