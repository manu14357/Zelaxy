...
/**
 * Build a single trace span from a BlockLog.
 * Used for incremental real-time log streaming — called once per block
 * as it completes, rather than waiting for the entire workflow to finish.
 */
export function buildSingleBlockTraceSpan(log: BlockLog): TraceSpan {
  if (!log.blockId || !log.blockType) {
    return {
      id: `unknown-${Date.now()}`,
      name: 'Unknown Block',
      type: 'unknown',
