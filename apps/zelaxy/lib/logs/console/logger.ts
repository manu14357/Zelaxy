...
const LOG_CONFIG = {
  development: {
    enabled: true,
    minLevel: LogLevel.INFO, // Show INFO+ by default; set LOG_LEVEL=DEBUG for verbose
    colorize: true,
  },
  production: {
...
const baseConfig = LOG_CONFIG[ENV] || LOG_CONFIG.development

// Allow overriding log level via LOG_LEVEL env var (e.g. LOG_LEVEL=DEBUG)
const envLogLevel = (typeof process !== 'undefined' && process.env?.LOG_LEVEL?.toUpperCase()) as
  | keyof typeof LogLevel
  | undefined
const config = {
  ...baseConfig,
  minLevel: envLogLevel && LogLevel[envLogLevel] ? LogLevel[envLogLevel] : baseConfig.minLevel,
}
