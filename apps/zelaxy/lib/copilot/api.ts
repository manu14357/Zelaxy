...
export interface SendMessageRequest {
  ...
  customApiKey?: string // Custom API key for the selected provider
}
...
/**
 * Send a message directly to a selected LLM provider without using the Python service
 * This is used when users select a custom provider/model
 * In agent mode, this now includes local tools for workflow creation
 */
export async function sendDirectMessage(request: SendMessageRequest): Promise<StreamingResponse> {
  try {
    const { abortSignal, ...requestBody } = request
    const response = await fetch('/api/copilot/direct-chat', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...requestBody,
        stream: true,
        mode: request.mode || 'agent', // Pass through mode for tool access
      }),
      signal: abortSignal,
      credentials: 'include',
    })
    ...
    if (!response.ok) {
      const errorMessage = await handleApiError(response, 'Failed to send direct message')
      throw new Error(errorMessage)
    }
    ...
    // Handle AbortError gracefully - this is expected when user aborts
    if (error instanceof Error && error.name === 'AbortError') {
      logger.info('Direct message was aborted by user')
      return {
        success: false,
        error: 'Request was aborted',
      }
