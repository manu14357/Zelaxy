  ...
  implements CopilotTool<TParams, TResult>
{
  ...
  async execute(params: TParams): Promise<CopilotToolResponse<TResult>> {
    ...
    try {
      ...
      // Check if the result itself has a success field (for results like BuildWorkflowResult)
      // If the inner result indicates failure, propagate that failure
      if (
        result &&
        ...
        const errorMessage =
          (result as any).message || (result as any).error || 'Tool operation failed'
        this.logger.warn(`Tool returned failure result: ${this.id}`, {
          toolId: this.id,
          error: errorMessage,
        })
        return {
          success: false,
          error: errorMessage,
          data: result, // Still include the full result for context
        }
      }
