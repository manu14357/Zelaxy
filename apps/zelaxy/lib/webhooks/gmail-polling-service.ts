...
export interface SimplifiedEmailAttachment {
  filename: string
  mimeType: string
  size: number
  attachmentId?: string
  /** Extracted text content for document attachments (PDF, etc.) */
  content?: string
}
...
export interface SimplifiedEmail {
  ...
  attachments: SimplifiedEmailAttachment[]
}
...
export async function pollGmailWebhooks() {
  logger.debug('Starting Gmail webhook polling')
  ...
  try {
    ...
    if (!activeWebhooks.length) {
      logger.debug('No active Gmail webhooks found')
    ...
    const enqueue = async (webhookData: (typeof activeWebhooks)[number]) => {
      ...
      try {
        ...
        if (!accessToken) {
          logger.debug(`[${requestId}] No Gmail access token for webhook ${webhookId}`)
    ...
    const summary = {
      ...
      successful: results.filter((r: any) => r.success === true).length,
      failed: results.filter((r: any) => r.success === false).length,
      details: results,
    }

    if (summary.failed > 0) {
      logger.warn('Gmail polling completed', {
        total: summary.total,
...
async function searchEmails(accessToken: string, config: GmailWebhookConfig, requestId: string) {
  try {
    ...
    // Use OR grouping with curly braces: {label:X label:Y} means X OR Y
    // Without braces, Gmail treats multiple label: as AND which requires ALL labels on one email
    const labelQuery =
      config.labelIds && config.labelIds.length > 0
        ? config.labelIds.length === 1
          ? `label:${config.labelIds[0]}`
    ...
    let query: string
    if (config.labelFilterBehavior === 'INCLUDE') {
      query = `${labelQuery}${timeConstraint}`
    } else {
      // EXCLUDE: negate each label individually — Gmail needs -label:X -label:Y
      const excludeQuery =
        config.labelIds && config.labelIds.length > 0
          ? config.labelIds.map((label) => `-label:${label}`).join(' ')
          : ''
      query = `${excludeQuery}${timeConstraint}`
    }
...
async function processEmails(
  ...
  for (const email of emails) {
    try {
      ...
      const attachments: SimplifiedEmailAttachment[] = []
      ...
      // Download attachments and extract text content for documents
      for (const attachment of attachments) {
        if (!attachment.attachmentId) continue
        try {
          const attUrl = `https://gmail.googleapis.com/gmail/v1/users/me/messages/${email.id}/attachments/${attachment.attachmentId}`
          const attResponse = await fetch(attUrl, {
            headers: { Authorization: `Bearer ${accessToken}` },
          })
          if (attResponse.ok) {
            const attData = (await attResponse.json()) as { data: string; size: number }
            attachment.size = attData.size || attachment.size

            // Convert base64url to standard base64
            const base64Str = attData.data.replace(/-/g, '+').replace(/_/g, '/')
            ...
            const docExtensions = ['docx', 'doc', 'xlsx', 'xls', 'csv', 'txt', 'md']
            const imageExtensions = ['png', 'jpg', 'jpeg', 'gif', 'bmp', 'tiff', 'svg']
            const isCadFile = cadExtensions.includes(ext)
            const isDocFile = docExtensions.includes(ext)
            const isImageFile =
              imageExtensions.includes(ext) || attachment.mimeType.startsWith('image/')

            if (isPdf) {
              try {
                const { parseBuffer } = await import('@/lib/file-parsers')
                const result = await parseBuffer(fileBuffer, 'pdf')
                attachment.content = result.content
                logger.info(
                  `[${requestId}] Extracted text from PDF attachment ${attachment.filename} (${result.content.length} chars)`
                )
              } catch (parseError) {
                logger.warn(
                  `[${requestId}] Failed to extract text from PDF ${attachment.filename}:`,
                  parseError
                )
                attachment.content = `[PDF file: ${attachment.filename}, ${attachment.size} bytes — text extraction failed]`
              }
            } else if (isCadFile) {
              // DWG, STEP, DXF, IGES — convert to PDF then extract text
              try {
                ...
                if (isSupportedFileType(ext)) {
                  ...
                  attachment.content = result.content
                  logger.info(
                    `[${requestId}] Extracted text from CAD attachment ${attachment.filename} (${result.content.length} chars)`
                  )
                } else {
                  attachment.content = `[${ext.toUpperCase()} file: ${attachment.filename}, ${attachment.size} bytes — no parser available]`
                }
              } catch (parseError) {
                logger.warn(
                  `[${requestId}] Failed to extract content from CAD file ${attachment.filename}:`,
                  parseError
                )
                attachment.content = `[${ext.toUpperCase()} file: ${attachment.filename}, ${attachment.size} bytes — conversion failed]`
              }
            } else if (isDocFile) {
              // DOCX, XLSX, CSV, TXT, MD — parse with file parsers
              try {
                const { parseBuffer, isSupportedFileType } = await import('@/lib/file-parsers')
                if (isSupportedFileType(ext)) {
                  const result = await parseBuffer(fileBuffer, ext)
                  attachment.content = result.content
                  logger.info(
                    `[${requestId}] Extracted text from document attachment ${attachment.filename} (${result.content.length} chars)`
                  )
                } else {
                  attachment.content = fileBuffer.toString('utf-8')
                }
              } catch (parseError) {
                logger.warn(
                  `[${requestId}] Failed to parse document ${attachment.filename}:`,
                  parseError
                )
                attachment.content = `[${ext.toUpperCase()} file: ${attachment.filename}, ${attachment.size} bytes — parsing failed]`
              }
            } else if (isImageFile) {
              // Images — try OCR via file converter → PDF → parse, or return metadata
              try {
                const { parseBuffer, isSupportedFileType } = await import('@/lib/file-parsers')
                if (isSupportedFileType(ext)) {
                  const result = await parseBuffer(fileBuffer, ext)
                  attachment.content = result.content
                  logger.info(
                    `[${requestId}] Extracted text from image attachment ${attachment.filename} (${result.content.length} chars)`
                  )
                } else {
                  attachment.content = `[Image file: ${attachment.filename}, ${attachment.mimeType}, ${attachment.size} bytes]`
                }
              } catch (parseError) {
                logger.warn(
                  `[${requestId}] Failed to process image ${attachment.filename}:`,
                  parseError
                )
                attachment.content = `[Image file: ${attachment.filename}, ${attachment.mimeType}, ${attachment.size} bytes]`
              }
            } else {
              // For non-PDF text-based files, try to extract as UTF-8 text
              const textMimeTypes = [
                'text/',
                'application/json',
                'application/xml',
              ...
              const isTextFile = textMimeTypes.some((t) => attachment.mimeType.startsWith(t))
              if (isTextFile) {
                attachment.content = fileBuffer.toString('utf-8')
              } else {
                attachment.content = `[Binary file: ${attachment.filename}, ${attachment.mimeType}, ${attachment.size} bytes]`
              }
            }
          } else {
            logger.warn(
              `[${requestId}] Failed to download attachment ${attachment.filename} for email ${email.id}: ${attResponse.status}`
            )
          }
        } catch (attError) {
          logger.warn(
            `[${requestId}] Error downloading attachment ${attachment.filename} for email ${email.id}:`,
            attError
          )
        }
      }
