...
export async function sendEmail({
  ...
  try {
    ...
    const senderEmail = from || `no-reply@${env.EMAIL_DOMAIN || getEmailDomain()}`
...
export async function sendBatchEmails({
  ...
  try {
    const senderEmail = `no-reply@${env.EMAIL_DOMAIN || getEmailDomain()}`
    ...
    const batchEmails = emails.map((email) => ({
      from: `Zelaxy <${email.from || senderEmail}>`,
      to: email.to,
