...
vi.mock('@/lib/env', () => ({
  env: {
    ...
    NEXT_PUBLIC_APP_URL: 'https://test.zelaxy.in',
  },
}))

vi.mock('@/lib/urls/utils', () => ({
  getEmailDomain: vi.fn().mockReturnValue('zelaxy.in'),
}))
...
describe('mailer', () => {
  const testEmailOptions = {
    ...
    html: '{{\x01}}Test email content{{\x01}}',
  }
  ...
  describe('sendEmail', () => {
    ...
    it('should send a marketing email with unsubscribe headers', async () => {
      const htmlWithToken = '{{\x01}}Test content{{\x01}}{{\x01}}Unsubscribe{{\x01}}'
    ...
    it.concurrent('should replace unsubscribe token placeholders in HTML', async () => {
      ...
      expect(mockSend).toHaveBeenCalledWith(
        expect.objectContaining({
          html: '{{\x01}}Content{{\x01}}{{\x01}}Unsubscribe{{\x01}}',
        })
      )
    })
  })
