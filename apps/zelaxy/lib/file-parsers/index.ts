...
function getParserInstances(): Record<string, FileParser> {
  if (parserInstances === null) {
    ...
    try {
      ...
      // ── DWG / DXF — dedicated WASM & JS parsers (no external CLI tools) ──
      // Only register on the server to avoid pulling Node.js modules into the client bundle.
      if (typeof window === 'undefined') {
        // DWG parser — lazy-loads @mlightcad/libredwg-web (WASM) on first call
        parserInstances.dwg = {
          async parseFile(filePath: string): Promise<FileParseResult> {
            const buffer = await readFile(filePath)
          ...
          async parseBuffer(buffer: Buffer): Promise<FileParseResult> {
            try {
              const { DwgParser } = await import('@/lib/file-parsers/dwg-parser')
              const parser = new DwgParser()
              return await parser.parseBuffer(buffer)
            } catch (err) {
              logger.error('DWG parser failed:', err)
              return {
                content: `[DWG file — ${buffer.length} bytes. Parser error: ${(err as Error).message}]`,
        ...
        // DXF parser — lazy-loads dxf-parser (pure JS) on first call
        parserInstances.dxf = {
          async parseFile(filePath: string): Promise<FileParseResult> {
            const buffer = await readFile(filePath)
            return this.parseBuffer!(buffer)
          ...
          async parseBuffer(buffer: Buffer): Promise<FileParseResult> {
            try {
              const { DxfNativeParser } = await import('@/lib/file-parsers/dxf-native-parser')
              const parser = new DxfNativeParser()
              return await parser.parseBuffer(buffer)
            } catch (err) {
              logger.error('DXF parser failed:', err)
              return {
                content: `[DXF file — ${buffer.length} bytes. Parser error: ${(err as Error).message}]`,
                metadata: { error: (err as Error).message },
              }
            }
          },
        ...
        // STEP / STP — dedicated metadata-only parser (avoids token overflow)
        const stepParserEntry: FileParser = {
          async parseFile(filePath: string): Promise<FileParseResult> {
            const buffer = await readFile(filePath)
            return this.parseBuffer!(buffer)
          },
          async parseBuffer(buffer: Buffer): Promise<FileParseResult> {
            try {
              const { StepParser } = await import('@/lib/file-parsers/step-parser')
              const parser = new StepParser()
              return await parser.parseBuffer(buffer)
            } catch (err) {
              logger.error('STEP parser failed:', err)
              return {
                content: `[STEP file — ${buffer.length} bytes. Parser error: ${(err as Error).message}]`,
                metadata: { error: (err as Error).message },
              }
            }
          },
        }
        parserInstances.step = stepParserEntry
        parserInstances.stp = stepParserEntry

        // IGES — use file-converter fallback (text-based format, much smaller than STEP)
        const createCadParser = (extension: string): FileParser => ({
          async parseFile(filePath: string): Promise<FileParseResult> {
            const buffer = await readFile(filePath)
            const filename = filePath.split('/').pop() || `file.${extension}`
            try {
              const { convertToPdf } = await import('@/lib/file-parsers/file-converter')
              const result = await convertToPdf(buffer, filename, 'application/octet-stream')

              if (result.mimeType === 'application/pdf' && parserInstances?.pdf?.parseBuffer) {
                return await parserInstances.pdf.parseBuffer(result.buffer)
              }
              ...
              return {
                content: `[${extension.toUpperCase()} file — ${buffer.length} bytes. Conversion method: ${result.method}]`,
                metadata: { conversionMethod: result.method },
              }
            } catch (error) {
              logger.error(`${extension.toUpperCase()} file parsing failed:`, error)
              return {
                content: `[${extension.toUpperCase()} file — ${buffer.length} bytes. Text extraction requires LibreOffice or FreeCAD.]`,
          ...
          async parseBuffer(buffer: Buffer): Promise<FileParseResult> {
            try {
              ...
              logger.error(`${extension.toUpperCase()} buffer parsing failed:`, error)
              return {
                content: `[${extension.toUpperCase()} file — ${buffer.length} bytes. Text extraction requires LibreOffice or FreeCAD.]`,
                metadata: { error: (error as Error).message },
              }
        ...
        parserInstances.iges = createCadParser('iges')
        parserInstances.igs = createCadParser('igs')
      }
    } catch (error) {
