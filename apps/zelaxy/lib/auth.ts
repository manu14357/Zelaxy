...
export const auth = betterAuth({
  ...
  session: {
    cookieCache: {
      enabled: false, // Disabled: avatar data URIs (~7KB) exceed cookie size limits
    },
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url, token }, request) => {
      ...
      const result = await resend.emails.send({
        from: `Zelaxy <hello@${env.EMAIL_DOMAIN || getEmailDomain()}>`,
        to: user.email,
  ...
  plugins: [
    ...
    emailOTP({
      sendVerificationOTP: async (data: {
        ...
        try {
          ...
          const result = await resend.emails.send({
            from: `Zelaxy <zelaxy@${env.EMAIL_DOMAIN || getEmailDomain()}>`,
            to: data.email,
    ...
    ...(isBillingEnabled && stripeClient
      ...
      : []),
    // Organization plugin - always enabled (works independently of billing)
    organization({
      // Allow team plan subscribers to create organizations when billing is enabled
      // When billing is disabled, allow all users to create organizations
      ...
      membershipLimit: 50,
      // Validate seat limits before sending invitations (only when billing is enabled)
      beforeInvite: async ({ organization }: { organization: { id: string } }) => {
        if (!isBillingEnabled) return // Skip seat validation when billing is disabled
      ...
      sendInvitationEmail: async (data: any) => {
        try {
          ...
          await resend.emails.send({
            from: `Zelaxy <hello@${env.EMAIL_DOMAIN || getEmailDomain()}>`,
            to: invitation.email,
            subject: `${inviterName} has invited you to join ${organization.name} on Zelaxy`,
            html,
          })
        } catch (error) {
