...
export async function getUserEntityPermissions(
  ...
  // For workspace entities, also check organization membership
  if (entityType === 'workspace') {
    ...
    if (orgId) {
      ...
      if (memberRecord.length > 0) {
        // Org owners/admins get admin permission on org workspaces
        // Regular members get read permission
        return ['owner', 'admin'].includes(memberRecord[0].role) ? 'admin' : 'read'
      }
    }
  }
