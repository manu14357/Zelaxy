...
export function useTableUndo({
  ...
  const deleteRowMutation = useDeleteTableRow(tableId)
  const deleteRowsMutation = useDeleteTableRows(tableId)
  ...
  const executeAction = useCallback(
        ...
        switch (action.type) {
          case 'update-cell': {
            ...
            updateRowMutation.mutate({
              rowId: action.rowId,
              data: { [action.columnName]: value } as RowData,
            })
            break
          }
                ...
                { data: action.data ?? ({} as RowData), workspaceId, position: action.position },
                {
                  onSuccess: (row: TableRow) => {
                    if (row.id && row.id !== action.rowId) {
                      patchUndoRowId(tableId, action.rowId, row.id)
                    }
                  },
                }
              )
          ...
          case 'create-rows': {
            if (direction === 'undo') {
              ...
              batchCreateRowsMutation.mutate(
                ...
                {
                  onSuccess: (result: { rows: TableRow[]; insertedCount: number }) => {
                  ...
                  onSuccess: () => {
                    if (action.cellData.length > 0) {
                      const updates = action.cellData.map((c) => ({
                        rowId: c.rowId,
                        data: { [action.columnName]: c.value } as RowData,
                      }))
                      void (async () => {
                        try {
                          for (
    ...
    [tableId, workspaceId, patchRedoRowId, patchUndoRowId]
  )
