import { ParallelAiIcon } from '@/components/icons'
import type { BlockConfig } from '@/blocks/types'
import type { ToolResponse } from '@/tools/types'

/**
 * Parallel AI — web research (search / extract / deep research). Named `parallel_ai` to avoid
 * colliding with the control-flow `parallel` block, which is a different concept.
 */
export const ParallelAiBlock: BlockConfig<ToolResponse> = {
  type: 'parallel_ai',
  name: 'Parallel AI',
  description: 'Web research with Parallel AI',
  longDescription:
    'Integrate Parallel AI into the workflow. Can search the web, extract information from URLs, and conduct deep research with citations.',
  docsLink: '#',
  category: 'tools',
  bgColor: '#1D1C1A',
  icon: ParallelAiIcon,
  ...
  outputs: {
    ...
    basis: {
      type: 'json',
      description: 'Citations and sources with field, reasoning, citations, confidence',
    },
  },
}
