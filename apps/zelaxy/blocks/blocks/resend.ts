...
Generate only valid JSON without explanations.`,
    ...
    // TRIGGER MODE: Trigger configuration (only shown when trigger mode is active)
    {
      ...
      type: 'trigger-config',
      layout: 'full',
      triggerProvider: 'resend',
      availableTriggers: ['resend_webhook'],
    },
  ],
  tools: {
  ...
  outputs: {
    ...
    event_type: { type: 'string', description: 'Resend event type (trigger events)' },
    email_id: { type: 'string', description: 'ID of the email' },
    to_email: { type: 'string', description: 'First recipient address' },
    subject: { type: 'string', description: 'Email subject' },
  },
  triggers: {
    enabled: true,
    available: ['resend_webhook'],
  },
}
