...
export const GmailBlock: BlockConfig<GmailToolResponse> = {
  ...
  subBlocks: [
    ...
    },
    {
      ...
      type: 'short-input',
      layout: 'full',
      placeholder: 'Enter specific message ID to fetch (overrides label)',
      condition: { field: 'operation', value: 'read_gmail' },
    },
    // Search Fields
    {
  ...
  outputs: {
    ...
    metadata: {
      type: 'json',
      description: 'Email metadata (id, threadId, from, to, subject, date, etc.)',
    },
    ...
    rawEmail: { type: 'json', description: 'Complete raw email data from Gmail API' },
  },
  triggers: {
    enabled: true,
