...
export const OutlookBlock: BlockConfig<OutlookResponse> = {
  ...
  subBlocks: [
    ...
    },
    {
      ...
      type: 'short-input',
      layout: 'full',
      placeholder: 'Enter specific message ID to fetch (overrides folder)',
      condition: { field: 'operation', value: 'read_outlook' },
    },
    {
      id: 'maxResults',
      ...
      type: 'short-input',
      layout: 'full',
      placeholder: 'Number of emails to retrieve (default: 1)',
      condition: { field: 'operation', value: 'read_outlook' },
    },
    {
  ...
  inputs: {
    ...
    messageId: { type: 'string', description: 'Specific message ID to fetch' },
    maxResults: { type: 'number', description: 'Maximum emails' },
    includeAttachments: { type: 'boolean', description: 'Include email attachments' },
  },
  outputs: {
    // Read operation outputs (structured email data)
    content: { type: 'string', description: 'Email body text content' },
    metadata: {
      type: 'json',
      description: 'Email metadata (id, from, to, cc, subject, date, etc.)',
    },
    attachments: { type: 'json', description: 'Email attachments array' },
    // Send/Draft operation outputs
    message: { type: 'string', description: 'Response message' },
    messageId: { type: 'string', description: 'Message or draft ID' },
    status: { type: 'string', description: 'Send/draft status' },
    // Trigger outputs
    email: {
      type: 'json',
      description: 'Email data from trigger (id, subject, from, to, body, attachments)',
    },
