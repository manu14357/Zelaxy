...
export class InputResolver {
  ...
  resolveVariableReferences(value: string, currentBlock?: SerializedBlock): string {
    ...
    for (const match of variableMatches) {
      const variableName = match.slice('{{variable.'.length, -2)
  ...
  resolveBlockReferences(
    ...
    // No need to check for XML conflicts since we're using {{}} syntax now
    ...
    for (const match of blockMatches) {
      // Skip variables - they've already been processed and use {{}} syntax now
      if (match.includes('variable.')) {
        continue
      }

      // Skip patterns that should be handled as environment variables
      // This prevents environment variables from being processed as block references
      const isApiKey = this.isApiKeyField(currentBlock, value)
      const isExplicitEnvVar = match.match(/^\{\{[^{}]+\}\}$/) && !match.includes('.')
      const hasProperEnvVarContext = this.containsProperEnvVarReference(value)

      if ((isApiKey || isExplicitEnvVar || hasProperEnvVarContext) && !match.includes('.')) {
        // This looks like an environment variable - let the env var resolver handle it
        continue
      }
      ...
      // No need to skip XML-like patterns since we're using {{}} syntax
      ...
      // If the template variable is inside a JSON string (surrounded by quotes),
      // escape characters that would break JSON: newlines, tabs, backslashes, quotes.
      // This ensures the resulting string stays valid JSON after interpolation.
      if (typeof replacementValue === 'string' && formattedValue.includes('\n')) {
        const matchIndex = resolvedValue.indexOf(match)
        if (matchIndex > 0) {
          const charBefore = resolvedValue[matchIndex - 1]
          const charAfter = resolvedValue[matchIndex + match.length]
          const looksLikeJson =
            resolvedValue.trimStart().startsWith('{') || resolvedValue.trimStart().startsWith('[')
          if (looksLikeJson && charBefore === '"' && (charAfter === '"' || charAfter === '\\')) {
            // Use JSON.stringify to escape, then strip the outer wrapping quotes
            formattedValue = JSON.stringify(formattedValue).slice(1, -1)
          }
        }
      }
  ...
  private containsProperEnvVarReference(value: string): boolean {
    ...
    // Case 1: String is just a single environment variable (but not a workflow variable or block reference)
    const singleVarMatch = value.trim().match(/^\{\{([^{}]+)\}\}$/)
    if (singleVarMatch && !value.includes('variable.') && !singleVarMatch[1].includes('.')) {
      return true
    }
    ...
    const properContextPatterns = [
      // Auth header patterns (excluding dot-containing references)
      /Bearer\s+\{\{[^.{}]+\}\}/i,
      /Authorization:\s+Bearer\s+\{\{[^.{}]+\}\}/i,
      /Authorization:\s+\{\{[^.{}]+\}\}/i,

      // API key in URL patterns (excluding dot-containing references)
      /[?&]api[_-]?key=\{\{[^.{}]+\}\}/i,
      /[?&]key=\{\{[^.{}]+\}\}/i,
      /[?&]token=\{\{[^.{}]+\}\}/i,

      // API key in header patterns (excluding dot-containing references)
      /X-API-Key:\s+\{\{[^.{}]+\}\}/i,
      /api[_-]?key:\s+\{\{[^.{}]+\}\}/i,
    ]

    // Additional check: make sure any matches don't contain dots
    return properContextPatterns.some((pattern) => {
      const match = value.match(pattern)
      if (match) {
        // Extract the content between {{ and }} and check it doesn't contain dots
        const envVarMatch = match[0].match(/\{\{([^}]+)\}\}/)
        return envVarMatch && !envVarMatch[1].includes('.')
      }
      return false
    })
  }
  ...
  resolveEnvVariables(value: any, isApiKey = false): any {
    if (typeof value === 'string') {
      ...
      // 2. String is a complete environment variable reference ({{ENV_VAR}}) but NOT a variable or block reference
      // 3. String contains environment variable references in proper contexts (auth headers, URLs)
      const isExplicitEnvVar =
      ...
      if (isApiKey || isExplicitEnvVar || hasProperEnvVarReferences) {
        // Only match environment variables that don't start with 'variable.' and don't contain dots (which are block references)
        const envMatches = value.match(/\{\{(?!variable\.)[^.{}]+\}\}/g)
        if (envMatches) {
          let resolvedValue = value
          for (const match of envMatches) {
            const envKey = match.slice(2, -2)
            // Skip if the key contains dots (block references)
            if (envKey.includes('.')) {
              continue
            }
   ...
   * Used to filter out non-block patterns from block reference resolution.
   *
