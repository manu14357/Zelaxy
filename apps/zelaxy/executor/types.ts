...
export interface UserFile {
  ...
  uploadedAt: string
  expiresAt: string
  /** Pre-extracted text content from file parsing (populated during upload) */
...
export interface ExecutionContext {
  ...
  // Loop settings configured in UI (maxIterations, parallelExecution, stopOnError)
  loopSettings?: Map<
    string,
    {
  ...
  // Parallel settings configured in UI (maxConcurrency, waitForAll, stopOnError, timeout)
  parallelSettings?: Map<
    string,
    {
  ...
  /**
   * Map of blockId -> enriched input data set by handlers after processing.
   * Used by the executor to update blockLog.input with the actual input
