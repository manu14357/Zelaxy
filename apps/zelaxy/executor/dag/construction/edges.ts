...
export class EdgeConstructor {
  ...
  private buildMetadataMaps(workflow: SerializedWorkflow): EdgeMetadata {
    ...
    for (const block of workflow.blocks) {
      ...
      if (isConditionBlockType(blockType)) {
        ...
        if (conditions) conditionConfigMap.set(block.id, conditions)
      } else if (isRouterV2BlockType(blockType)) {
        const routes = this.parseRouterV2Config(block)
        if (routes) routerV2ConfigMap.set(block.id, routes)
  ...
  private parseConditionConfig(block: any): ConditionConfig[] | null {
    try {
      ...
      if (typeof conditionsJson === 'string') return JSON.parse(conditionsJson)
      if (Array.isArray(conditionsJson)) return conditionsJson
      return null
    } catch (error) {
  ...
  private parseRouterV2Config(block: any): RouterV2RouteConfig[] | null {
    try {
      ...
      if (typeof routesJson === 'string') return JSON.parse(routesJson)
      if (Array.isArray(routesJson)) return routesJson
      return null
    } catch (error) {
  ...
  private wireRegularEdges(
    ...
    for (const connection of workflow.connections) {
      ...
      if (sourceIsLoopBlock) {
        ...
        if (!dag.nodes.has(sentinelEndId) || !dag.nodes.has(loopSentinelStartId)) continue
      ...
      if (!this.isEdgeReachable(source, target, reachableBlocks, dag)) continue
  ...
  private wireLoopSentinels(dag: DAG): void {
    for (const [loopId, loopConfig] of dag.loopConfigs) {
      ...
      if (!dag.nodes.has(sentinelStartId) || !dag.nodes.has(sentinelEndId)) continue
  ...
  private wireParallelSentinels(dag: DAG): void {
    for (const [parallelId, parallelConfig] of dag.parallelConfigs) {
      ...
      for (const entryNodeId of entryNodes) {
        ...
        if (dag.nodes.has(targetId)) this.addEdge(dag, sentinelStartId, targetId)
      }
  ...
  private resolveSubflowToSentinelStart(nodeId: string, dag: DAG): string {
    if (dag.parallelConfigs.has(nodeId)) return buildParallelSentinelStartId(nodeId)
  ...
  private resolveSubflowToSentinelEnd(nodeId: string, dag: DAG): string {
    if (dag.parallelConfigs.has(nodeId)) return buildParallelSentinelEndId(nodeId)
    if (dag.loopConfigs.has(nodeId)) return buildSentinelEndId(nodeId)
    return buildBranchNodeId(nodeId, 0)
  }
  ...
  private edgeCrossesLoopBoundary(
    ...
    if (sourceInLoop !== targetInLoop) return true
    if (!sourceInLoop && !targetInLoop) return false
  ...
  private findInnermostLoop(blockId: string, dag: DAG): string | undefined {
    ...
    for (const [loopId, loopConfig] of dag.loopConfigs) {
      if (loopConfig.nodes.includes(blockId)) candidates.push(loopId)
    }
  ...
  private isEdgeReachable(
    ...
    if (!reachableBlocks.has(source) && !dag.nodes.has(source)) return false
    if (!reachableBlocks.has(target) && !dag.nodes.has(target)) return false
    return true
  }
  ...
  private findLoopBoundaryNodes(
    ...
    for (const nodeId of nodes) {
      ...
      if (!hasIncomingFromLoop) startNodesSet.add(nodeId)
    }
    ...
    for (const nodeId of nodes) {
      ...
      if (!hasOutgoingToLoop) terminalNodesSet.add(nodeId)
    }
  ...
  private findParallelBoundaryNodes(
    ...
    for (const nodeId of nodes) {
      ...
      if (startNode) {
        ...
        if (!hasIncomingFromParallel) entryNodes.push(nodeId)
      }
      ...
      if (endNode) {
        ...
        if (!hasOutgoingToParallel) terminalNodes.push(nodeId)
      }
    }
  ...
  private getParallelId(blockId: string, dag: DAG): string | null {
    for (const [parallelId, parallelConfig] of dag.parallelConfigs) {
      if (parallelConfig.nodes.includes(blockId)) return parallelId
    }
    return null
  }
