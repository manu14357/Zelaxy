 ...
 * Note: Telemetry is disabled in this privacy-focused fork
 */
function trackWorkflowTelemetry(eventName: string, data: Record<string, any>) {
  // Telemetry disabled for privacy
  // All tracking and analytics have been removed from this fork
}
...
export class Executor {
  ...
  constructor(
    ...
    this.validateWorkflow(constructorStartBlockId)
  ...
  async execute(
    ...
    try {
      await this.onExecutionStart?.(workflowId, this.contextExtensions.executionId)
    } catch (e) {
    ...
    try {
      ...
      try {
        await this.onExecutionComplete?.(successResult)
      } catch (e) {
      ...
      try {
        await this.onExecutionComplete?.(errorResult)
      } catch (e) {
        logger.warn('onExecutionComplete callback error:', e)
      }
          ...
          for (const field of inputFormat) {
            if (field.name && field.type) {
              ...
              if (
                ...
                typeof this.workflowInput.input === 'object' &&
                !Array.isArray(this.workflowInput.input) &&
                this.workflowInput.input[field.name] !== undefined
              ) {
                inputValue = this.workflowInput.input[field.name]
              }
              // Then try direct field access
              else if (this.workflowInput?.[field.name] !== undefined) {
                inputValue = this.workflowInput[field.name]
              }
              ...
              // and this is the first/primary field, use the input value directly
                ...
                inputValue = this.workflowInput.input
              }
  ...
  private async executeBlock(
    ...
    try {
      ...
      if (
        ...
        try {
          await this.onBlockComplete?.(blockLog)
        } catch (e) {
