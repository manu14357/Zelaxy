...
export class FileToolProcessor {
  ...
  private static async processFileData(
    ...
    try {
      // Guard: If this is already a processed UserFile (has 'key' from S3 upload),
      // skip re-processing to avoid downloading, re-uploading, and re-parsing.
      // This can happen when the proxy already processed file outputs and the
      // caller invokes processFileOutputs again.
      if (
        fileData &&
        typeof fileData === 'object' &&
        'key' in fileData &&
        typeof (fileData as any).key === 'string' &&
        (fileData as any).key.length > 0
      ) {
        logger.info(
          `Skipping already-processed UserFile for '${outputKey}': ${fileData.name} (key: ${(fileData as any).key})`
        )
        return fileData as unknown as UserFile
      }
      ...
      // Parse file content so downstream blocks (e.g. agent) can read extracted text.
      // This mirrors the logic in /api/files/upload for starter-block uploads.
      try {
        const ext = fileData.name.includes('.')
        ...
        if (ext && isSupportedFileType(ext)) {
          ...
          if (parseResult.content) {
            ...
            userFile.parsedContent =
              parseResult.content.length > MAX_CONTENT
                ? parseResult.content.substring(0, MAX_CONTENT) +
                  `\n[Content truncated at ${MAX_CONTENT} chars]`
                : parseResult.content
            logger.info(
              `Parsed file content for ${fileData.name}: ${userFile.parsedContent.length} chars`
            )
          }
        }
      } catch (parseError) {
        logger.warn(`Failed to parse file content for ${fileData.name}:`, parseError)
      }
