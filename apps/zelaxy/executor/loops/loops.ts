  ...
  async processLoopIterations(context: ExecutionContext): Promise<boolean> {
    ...
    for (const [loopId, loop] of Object.entries(this.loops)) {
      ...
      if (allBlocksInLoopExecuted) {
        ...
        // Get loop settings (stopOnError, parallelExecution)
        const loopSettings = context.loopSettings?.get(loopId)
        const stopOnError = loopSettings?.stopOnError ?? false

        // Check for errors in this iteration's blocks
        ...
        for (const nodeId of loop.nodes) {
          ...
          if (blockState?.output?.error) {
            ...
            logger.warn(
              `Loop ${loopId} - Block ${nodeId} had an error in iteration ${currentIteration}:`,
              blockState.output.error
            )
            break
          }
        }

        // If stopOnError is enabled and there was an error, complete the loop early
        if (stopOnError && hasIterationError) {
          logger.info(`Loop ${loopId} - Stopping early due to error (stopOnError enabled)`)
          hasLoopReachedMaxIterations = true
          context.completedLoops.add(loopId)

          // Store early completion state
          const aggregatedOutput = {
            ...
            completed: true,
            stoppedEarly: true,
            error: 'Stopped due to error in iteration',
            message: `Stopped at iteration ${currentIteration} due to error`,
          }
          ...
          // Activate post-loop connections even on early stop
          ...
          for (const conn of loopEndConnections) {
            ...
            logger.info(`Activated post-loop path from ${loopId} to ${conn.target} (early stop)`)
          }
        ...
        let maxIterations = Number(loop.iterations) || this.defaultIterations
