...
export class ParallelManager {
  ...
  async processParallelIterations(context: ExecutionContext): Promise<void> {
    ...
    for (const [parallelId, parallel] of Object.entries(this.parallels)) {
      ...
      // Get parallel settings (stopOnError)
      const parallelSettings = context.parallelSettings?.get(parallelId)
      const stopOnError = parallelSettings?.stopOnError ?? false

      // Check for errors in any virtual block
      let hasParallelError = false
      let errorMessage = ''
      if (stopOnError) {
        for (const nodeId of parallel.nodes) {
          for (let i = 0; i < parallelState.parallelCount; i++) {
            const virtualBlockId = `${nodeId}_parallel_${parallelId}_iteration_${i}`
            const blockState = context.blockStates.get(virtualBlockId)
            if (blockState?.output?.error) {
              hasParallelError = true
              errorMessage = blockState.output.error
              logger.warn(
                `Parallel ${parallelId} - Virtual block ${virtualBlockId} had an error:`,
                errorMessage
              )
              break
            }
          }
          if (hasParallelError) break
        }

        // If stopOnError and there's an error, mark as completed early
        if (hasParallelError) {
          logger.info(`Parallel ${parallelId} - Stopping early due to error (stopOnError enabled)`)
          ...
          const aggregatedOutput = {
            ...
            stoppedEarly: true,
            error: errorMessage,
            message: `Stopped due to error in parallel execution`,
          }

          context.blockStates.set(parallelId, {
            output: aggregatedOutput,
            executed: true,
            executionTime: 0,
          })

          // Activate post-parallel connections
          ...
          for (const conn of parallelEndConnections) {
            ...
            logger.info(`Activated post-parallel path to ${conn.target} (early stop)`)
          }
