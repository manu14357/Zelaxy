...
import type { DAG, DAGEdge, DAGNode } from '@/executor/dag/builder'
import type { NormalizedBlockOutput } from '@/executor/types'

const logger = createLogger('DAGEdgeManager')

/** Loop/parallel control handles — driven by sentinel outputs, not regular data flow. */
const SUBFLOW_CONTROL_EDGE_HANDLES = new Set<string>([
  EDGE.LOOP_CONTINUE,
...
/** Handles that point backwards (end sentinel → loop body) and must not be walked forward. */
...
/**
 * Decides which downstream nodes become ready after a node completes, by activating the edges whose
 * routing condition the node's output satisfies and deactivating (and cascading) the rest.
 *
 * Reads Zelaxy's handler output shape: condition handles match `output.selectedConditionId`, router
 * handles match `output.selectedPath.blockId`; loop/parallel control handles match
 * `output.selectedRoute` emitted by the sentinels.
 */
export class EdgeManager {
  ...
  processOutgoingEdges(
    ...
    const isRoutedDeadEnd = isDeadEnd && !!(output.selectedPath || output.selectedRoute)
  ...
  /**
   * Clears deactivated edges whose source is in the given set (used when restoring loop state for
   * the next iteration). Edges pointing into the set from outside stay deactivated so the loop
   * doesn't stall waiting on a source that will never fire again.
   */
  clearDeactivatedEdgesForNodes(nodeIds: Set<string>): void {
    ...
    for (const edgeKey of this.deactivatedEdges) {
      ...
      if (nodeIds.has(sourceId)) edgesToRemove.push(edgeKey)
    }
  ...
  /** Marks a single edge inactive so its target no longer waits on it. */
  deactivateEdge(sourceId: string, targetId: string, sourceHandle?: string): void {
  ...
  /**
   * Whether the cascade-target sentinel encloses the source node (same loop/parallel). A condition
   * inside a loop that hits a dead end should still let the enclosing loop's sentinel fire.
   */
  ...
  private shouldActivateEdge(edge: DAGEdge, output: NormalizedBlockOutput): boolean {
    ...
    if (handle.startsWith(EDGE.ROUTER_PREFIX)) {
      ...
      return output.selectedPath?.blockId === routeId || output.selectedRoute === routeId
    }
  ...
  private parseEdgeKey(edgeKey: string): { sourceId: string; targetId: string } | null {
    let parsed: unknown
    try {
    ...
    if (
      ...
      return { sourceId: parsed[0], targetId: parsed[1] }
    }
    return null
  }
}
