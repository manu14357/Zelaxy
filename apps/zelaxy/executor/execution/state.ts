...
export interface BlockStateController {
...
export class ExecutionState implements BlockStateController {
  ...
  getBlockOutput(blockId: string, currentNodeId?: string): NormalizedBlockOutput | undefined {
    ...
    if (normalizedId !== blockId) return undefined
    ...
    if (currentNodeId) {
      ...
      if (suffixedOutput !== undefined) return suffixedOutput
    }

    for (const [storedId, state] of this.blockStates.entries()) {
      if (normalizeLookupId(storedId) === blockId) return state.output
    }
