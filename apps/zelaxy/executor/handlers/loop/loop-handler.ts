...
export class LoopBlockHandler implements BlockHandler {
  ...
  async execute(
    ...
    // Read loop configuration properties from block.config.params
    const blockParams = block.config?.params || {}
    const configuredMaxIterations = blockParams.maxIterations
    const parallelExecution = blockParams.parallelExecution === true
    const stopOnError = blockParams.stopOnError === true

    logger.info(`Loop ${block.id} - Config params:`, {
      configuredMaxIterations,
      parallelExecution,
      stopOnError,
      count: blockParams.count,
    })
    ...
    if (loop.loopType === 'forEach') {
      ...
      // For count-based loops, use configured iterations or default
      // Values may be strings from subblock inputs, so parse as numbers
      maxIterations = Number(loop.iterations) || Number(blockParams.count) || DEFAULT_MAX_ITERATIONS
      logger.info(`For loop ${block.id} - Configured iterations: ${maxIterations}`)
    }

    // Apply maxIterations limit from UI slider (if configured)
    if (
      configuredMaxIterations &&
      typeof configuredMaxIterations === 'number' &&
      configuredMaxIterations > 0
    ) {
      if (maxIterations > configuredMaxIterations) {
        logger.info(
          `Loop ${block.id} - Capping iterations from ${maxIterations} to configured limit ${configuredMaxIterations}`
        )
        maxIterations = configuredMaxIterations
      }
    }

    // Store loop execution settings in context for use by loop manager
    if (!context.loopSettings) {
      context.loopSettings = new Map()
    }
    context.loopSettings.set(block.id, {
      parallelExecution,
      stopOnError,
      maxIterations,
    })
