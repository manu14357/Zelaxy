...
export class WorkflowBlockHandler implements BlockHandler {
  ...
  async execute(
    ...
    try {
      ...
      // Always wrap in { input: ... } so the child starter block exposes `start.input`
      let childWorkflowInput: Record<string, any>

      if (inputs.input !== undefined && inputs.input !== null && inputs.input !== '') {
        // If input is provided, wrap it so the child executor maps it to start.input
        childWorkflowInput = { input: inputs.input }
        logger.info(`Passing input to child workflow: ${JSON.stringify(childWorkflowInput)}`)
      } else {
        // Even when no input is provided, ensure start.input exists in the child workflow
        // so references to {{start.input}} don't fail with "No value found"
        childWorkflowInput = { input: '' }
        logger.info('No input provided for child workflow, using empty string as default')
      }
