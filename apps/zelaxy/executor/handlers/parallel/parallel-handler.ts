...
export class ParallelBlockHandler implements BlockHandler {
  ...
  async execute(
    ...
    if (!parallelState) {
      ...
      // Read parallel configuration properties from block.config.params
      const blockParams = block.config?.params || {}
      const maxConcurrency =
        typeof blockParams.maxConcurrency === 'number' ? blockParams.maxConcurrency : 20
      const waitForAll = blockParams.waitForAll !== false // Default true
      const stopOnError = blockParams.stopOnError === true
      const timeout = typeof blockParams.timeout === 'number' ? blockParams.timeout * 1000 : 0 // Convert to ms

      logger.info(`Parallel ${block.id} - Config params:`, {
        maxConcurrency,
        waitForAll,
        stopOnError,
        timeout,
        count: blockParams.count,
        parallelType: blockParams.parallelType,
      })
      ...
      let parallelType = parallel.parallelType || blockParams.parallelType
      if (!parallelType) {
      ...
      const countValue = Number(parallel.count) || Number(blockParams.count) || 1
      ...
      if (parallelType === 'count') {
        ...
        parallelCount = Math.max(1, countValue)
      ...
      // Store parallel settings in context for use by parallel manager
      if (!context.parallelSettings) {
        context.parallelSettings = new Map()
      }
      context.parallelSettings.set(block.id, {
        maxConcurrency,
        waitForAll,
        stopOnError,
        timeout,
