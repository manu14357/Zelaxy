...
export class Serializer {
  serializeWorkflow(
    ...
    return {
      ...
      blocks: Object.values(blocks)
        .map((block) => this.serializeBlock(block, validateRequired))
  ...
  private serializeBlock(block: BlockState, validateRequired = false): SerializedBlock | null {
    ...
    if (block.type === 'loop' || block.type === 'parallel') {
      return {
        ...
        metadata: {
          ...
          category: 'subflow',
          color: block.type === 'loop' ? '#F97316' : '#EA580C',
        },
    ...
    if (!blockConfig) {
      logger.warn(`Skipping unknown block type: ${block.type}`, {
        blockId: block.id,
