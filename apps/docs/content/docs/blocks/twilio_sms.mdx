...
- Trigger a workflow automatically when your Twilio number receives an SMS or MMS
...
- **Type**: trigger-config
- **Available triggers**: `twilio_webhook`

When the block runs in trigger mode, this configures the `twilio_webhook` trigger so incoming SMS/MMS start the workflow. Deliveries are verified against the `X-Twilio-Signature` header.
See the [Twilio SMS webhook trigger](/docs/triggers/webhook#twilio-sms) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event instead: `from` (string), `to` (string), `body` (string), `message_sid` (string), `message_status` (string), `num_media` (number), `media` (json), `from_city` (string), `raw` (json). See the [Twilio SMS webhook trigger](/docs/triggers/webhook#twilio-sms) for the full schema.
