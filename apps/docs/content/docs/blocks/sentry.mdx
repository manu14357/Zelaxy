...
- Trigger a workflow automatically when a Sentry issue is created, resolved, or an alert fires
...
- **Type**: trigger-config
- **Available triggers**: `sentry_webhook`

When the block runs in trigger mode, this configures the `sentry_webhook` trigger so Sentry events start the workflow. The client secret is verified against the `Sentry-Hook-Signature` header on every delivery.
See the [Sentry webhook trigger](/docs/triggers/webhook#sentry) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event instead: `action` (string), `resource` (string), `issue_id` (string), `issue_title` (string), `issue_url` (string), `short_id` (string), `culprit` (string), `level` (string), `status` (string), `event_count` (string), `user_count` (number), `project_slug` (string), `error_message` (string), `environment` (string). See the [sentry webhook trigger](/docs/triggers/webhook#sentry) for the full schema.
