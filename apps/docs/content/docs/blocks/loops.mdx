...
- Trigger a workflow when an email is delivered, opened, clicked, or bounces
...
- **Type**: trigger-config
- **Available triggers**: `loops_webhook`

When the block runs in trigger mode, this configures the `loops_webhook` trigger so Loops events start the workflow.
See the [Loops webhook trigger](/docs/triggers/webhook#loops) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `email` (string), `campaign_name` (string), `link_url` (string). See the [Loops webhook trigger](/docs/triggers/webhook#loops) for the full schema.
