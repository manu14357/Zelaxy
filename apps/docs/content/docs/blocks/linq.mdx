...
- Trigger a workflow when a message is delivered, fails, or gets a reply
...
- **Type**: trigger-config
- **Available triggers**: `linq_webhook`

When the block runs in trigger mode, this configures the `linq_webhook` trigger so Linq events start the workflow.
See the [Linq webhook trigger](/docs/triggers/webhook#linq) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `message_id` (string), `status` (string), `body` (string). See the [Linq webhook trigger](/docs/triggers/webhook#linq) for the full schema.
