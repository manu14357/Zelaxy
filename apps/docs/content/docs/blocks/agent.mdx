...
Supported providers include: OpenAI, Anthropic, Azure Anthropic, Google, Google Vertex AI, xAI, DeepSeek, Groq, Cerebras, Azure OpenAI, Mistral, Perplexity, Cohere, OpenRouter, NVIDIA, Fireworks, Together, Z.ai, Sakana AI, Meta, Xiaomi MiMo, Xiaomi MiMo Token Plan, vLLM, LiteLLM, Baseten, Ollama, and Ollama Cloud.

Azure Anthropic runs Claude models through an Azure AI Foundry deployment (set `AZURE_ANTHROPIC_ENDPOINT` / `AZURE_ANTHROPIC_API_VERSION`, or the Agent block's Azure fields), and Google Vertex AI runs Gemini through a GCP project (set `VERTEX_PROJECT` / `VERTEX_LOCATION`) — the enterprise procurement paths for teams that can't use the public Anthropic/Gemini API keys.

Z.ai (GLM models — set `ZAI_API_KEY`), Sakana AI (Fugu models — `SAKANA_API_KEY`), and Meta (Muse Spark via the Meta Model API — `META_API_KEY`) are OpenAI-compatible endpoints; select any of their models from the dropdown and provide the matching API key.
...
|----------|----------|-------------------|---------|
| **MiMo** (pay-as-you-go) | `https://api.xiaomimimo.com/v1` | `XIAOMI_MIMO_API_KEY` | Per-token against your account balance ([pay-as-you-go pricing](https://mimo.mi.com/docs/en-US/price/pay-as-you-go)) |
| **MiMo Token Plan** (subscription) | `https://token-plan-sgp.xiaomimimo.com/v1` | `XIAOMI_MIMO_TOKEN_PLAN_API_KEY` | Prepaid subscription quota metered in **Credits** ([Token Plan pricing](https://mimo.mi.com/docs/en-US/price/token-plan)) |
