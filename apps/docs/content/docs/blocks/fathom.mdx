...
- Trigger a workflow when a Fathom meeting summary or transcript is ready
...
- **Type**: trigger-config
- **Available triggers**: `fathom_webhook`

When the block runs in trigger mode, this configures the `fathom_webhook` trigger so Fathom events start the workflow.
See the [Fathom webhook trigger](/docs/triggers/webhook#fathom) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `meeting_id` (string), `meeting_title` (string), `summary` (string), `transcript` (string). See the [Fathom webhook trigger](/docs/triggers/webhook#fathom) for the full schema.
