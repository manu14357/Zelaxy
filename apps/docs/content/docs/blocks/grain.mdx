...
- Trigger a workflow when a Grain recording completes or a highlight is created
...
- **Type**: trigger-config
- **Available triggers**: `grain_webhook`

When the block runs in trigger mode, this configures the `grain_webhook` trigger so Grain events start the workflow.
See the [Grain webhook trigger](/docs/triggers/webhook#grain) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `recording_id` (string), `recording_title` (string), `recording_url` (string). See the [Grain webhook trigger](/docs/triggers/webhook#grain) for the full schema.
