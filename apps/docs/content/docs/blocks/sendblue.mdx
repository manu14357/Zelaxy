...
- Trigger a workflow when an iMessage/SMS is received or its status changes
...
- **Type**: trigger-config
- **Available triggers**: `sendblue_webhook`

When the block runs in trigger mode, this configures the `sendblue_webhook` trigger so Sendblue events start the workflow.
See the [Sendblue webhook trigger](/docs/triggers/webhook#sendblue) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `from_number` (string), `content` (string), `status` (string). See the [Sendblue webhook trigger](/docs/triggers/webhook#sendblue) for the full schema.
