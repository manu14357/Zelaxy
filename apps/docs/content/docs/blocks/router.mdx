...
- **Type**: `dropdown`
- **Required**: yes
- **Options**: dynamically populated from all registered base model providers plus any locally running Ollama models. Examples include `gpt-4o`, `gpt-4o-mini`, `claude-3-5-sonnet-20241022`, `gemini-1.5-pro`, `deepseek-chat`, `deepseek-reasoner`, `grok-2-1212`, and any Ollama model the user has pulled.
...
- **OpenAI Chat** (`openai_chat`) — Routes via an OpenAI model (e.g. `gpt-4o`, `gpt-4o-mini`)
- **Anthropic Chat** (`anthropic_chat`) — Routes via an Anthropic model (e.g. `claude-3-5-sonnet-20241022`)
- **Google Chat** (`google_chat`) — Routes via a Google model (e.g. `gemini-1.5-pro`)
- **xAI Chat** (`xai_chat`) — Routes via an xAI model (e.g. `grok-2-1212`)
