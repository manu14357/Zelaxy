...
- Trigger a workflow automatically when your Twilio number receives a call or a call status changes
...
- **Type**: trigger-config
- **Available triggers**: `twilio_voice_webhook`

When the block runs in trigger mode, this configures the `twilio_voice_webhook` trigger so Twilio Voice events start the workflow. Deliveries are verified against the `X-Twilio-Signature` header.
See the [Twilio Voice webhook trigger](/docs/triggers/webhook#twilio-voice) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event instead: `from` (string), `to` (string), `call_sid` (string), `call_status` (string), `direction` (string), `call_duration` (string), `recording_url` (string), `raw` (json). See the [Twilio Voice webhook trigger](/docs/triggers/webhook#twilio-voice) for the full schema.
