...
- Trigger a workflow when a purchase, renewal, cancellation, or expiration happens
...
- **Type**: trigger-config
- **Available triggers**: `revenuecat_webhook`

When the block runs in trigger mode, this configures the `revenuecat_webhook` trigger so RevenueCat events start the workflow.
See the [RevenueCat webhook trigger](/docs/triggers/webhook#revenuecat) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `app_user_id` (string), `product_id` (string), `store` (string), `environment` (string). See the [RevenueCat webhook trigger](/docs/triggers/webhook#revenuecat) for the full schema.
