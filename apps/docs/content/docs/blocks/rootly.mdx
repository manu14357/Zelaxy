...
- Trigger a workflow when an incident is created, mitigated, or resolved
...
- **Type**: trigger-config
- **Available triggers**: `rootly_webhook`

When the block runs in trigger mode, this configures the `rootly_webhook` trigger so Rootly events start the workflow.
See the [Rootly webhook trigger](/docs/triggers/webhook#rootly) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `incident_id` (string), `incident_title` (string), `severity` (string), `url` (string). See the [Rootly webhook trigger](/docs/triggers/webhook#rootly) for the full schema.
