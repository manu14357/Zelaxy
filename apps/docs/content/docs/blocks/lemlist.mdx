...
- **Type**: trigger-config
- **Available triggers**: `lemlist_webhook`

When the block runs in trigger mode, this configures the `lemlist_webhook` trigger so lemlist events start the workflow.
See the [lemlist webhook trigger](/docs/triggers/webhook#lemlist) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `lead_email` (string), `campaign_name` (string), `text` (string). See the [lemlist webhook trigger](/docs/triggers/webhook#lemlist) for the full schema.
