...
- Trigger a workflow when an incident is created, updated, or resolved
...
- **Type**: trigger-config
- **Available triggers**: `incidentio_webhook`

When the block runs in trigger mode, this configures the `incidentio_webhook` trigger so incident.io events start the workflow.
See the [incident.io webhook trigger](/docs/triggers/webhook#incident-io) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `incident_id` (string), `incident_name` (string), `severity` (string), `permalink` (string). See the [incident.io webhook trigger](/docs/triggers/webhook#incident-io) for the full schema.
