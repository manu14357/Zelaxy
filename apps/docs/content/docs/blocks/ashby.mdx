...
- Trigger a workflow when an application is submitted or a candidate changes stage
...
- **Type**: trigger-config
- **Available triggers**: `ashby_webhook`

When the block runs in trigger mode, this configures the `ashby_webhook` trigger so Ashby events start the workflow.
See the [Ashby webhook trigger](/docs/triggers/webhook#ashby) for the full output schema and setup steps.
...
When running in trigger mode the block emits the webhook event: `event_type` (string), `candidate_name` (string), `candidate_email` (string), `job_title` (string), `stage` (string). See the [Ashby webhook trigger](/docs/triggers/webhook#ashby) for the full schema.
