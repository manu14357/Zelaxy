...
When the block runs in trigger mode, this configures the `calendly_webhook` trigger so booking events start the workflow. The signing key is verified against the `Calendly-Webhook-Signature` header on every delivery.
See the [Calendly webhook trigger](/docs/triggers/webhook#calendly) for the full output schema and setup steps.
