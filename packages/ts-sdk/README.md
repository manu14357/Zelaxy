# Zelaxy TypeScript SDK

The official TypeScript/JavaScript SDK for [Zelaxy](http://localhost:3000/), allowing you to execute workflows programmatically from your applications.
...
```typescript
new ZelaxyClient(config: ZelaxyConfig)
```

- `config.apiKey` (string): Your Zelaxy API key
- `config.baseUrl` (string, optional): Base URL for the Zelaxy API (defaults to `http://localhost:3000/`)
...
```typescript
class ZelaxyError extends Error {
  code?: string;
  status?: number;
}
```
...
const client = new ZelaxyClient({
  apiKey: process.env.ZELAXY_API_KEY!,
  baseUrl: process.env.ZELAXY_BASE_URL // optional
});
```
