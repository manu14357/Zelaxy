"""
Zelaxy SDK for Python

Official Python SDK for Zelaxy, allowing you to execute workflows programmatically.
"""
...
class ZelaxyError(Exception):
    """Exception raised for Zelaxy API errors."""
...
class ZelaxyClient:
    """
    Zelaxy API client for executing workflows programmatically.

    Args:
        api_key: Your Zelaxy API key
        base_url: Base URL for the Zelaxy API (defaults to http://localhost:3000/)
    """
    ...
    def execute_workflow(
        ...
        Raises:
            ZelaxyError: If the workflow execution fails
        """
        ...
        try:
            ...
            if not response.ok:
                ...
                raise ZelaxyError(error_message, error_code, response.status_code)
            ...
            raise ZelaxyError(f'Workflow execution timed out after {timeout} seconds', 'TIMEOUT')
        except requests.RequestException as e:
            raise ZelaxyError(f'Failed to execute workflow: {str(e)}', 'EXECUTION_ERROR')
    ...
    def get_workflow_status(self, workflow_id: str) -> WorkflowStatus:
        ...
        Raises:
            ZelaxyError: If getting the status fails
        """
        ...
        try:
            ...
            raise ZelaxyError(f'Failed to get workflow status: {str(e)}', 'STATUS_ERROR')
